...
// TODO : ESM use... export class CachierDB extends Cachier {
  ...
  async register(data, read, write) {
    ...
    try {
      ...
      rtn = super.register(data, read, write);
    } finally {
      if (read || write) {
        rtn = await rtn;
...
async function dbReader(name, path, ext, forContent, optional, params, store, readFormatter, close, log, isCompile, recordFunc, openFunc) {
  let rtn;
  if (log.debug) {
    log.debug(`Reading template "${forContent ? 'data' : 'sources'}" from DB for "${name}" @ "${path}" (${isCompile ? 'compile' : 'render'}-time)`);
  }
  try {
...
async function dbWriter(name, path, ext, forContent, optional, params, store, data, writeFormatter, close, log, isCompile, recordFunc, openFunc) {
  ...
  if (log.debug) {
    log.debug(`Writting template "${forContent ? 'data' : 'sources'}" to DB for "${name}" @ "${path}" (${isCompile ? 'compile' : 'render'}-time)`);
  }
...
async function dbFinish(store, optional, log) {
  ...
  if (log.info) log.info('Releasing resources for DB (render-time)');
...
async function execDB(name, optional, params, storage, formatter, close, log, isCompile, valueOrRemove, recordfunc, openFunc, openOnly) {
  ...
  if (!name && (!policy.includes('read-all-on-init-when-empty') || (storage.data && Object.getOwnPropertyNames(storage.data).length))) {
    if (log && log.info) {
        ...
        if (remove && dels) {
          for (let del of dels) {
            try {
              ...
              if (log && log.info) {
                log.info(`Completed "delete" on "${del.cacheName}" for IndexedDB key "${del.key}" (${isCompile ? 'compile' : 'render'}-time)`);
              }
            } catch (err) {
      ...
      if (name) { // single record processing
        try {
          if (remove) {
            ...
            if (log && log.info) log.info(`Completed "delete" for LevelDB key "${name}" (${isCompile ? 'compile' : 'render'}-time)`);
            return end();
          }
          let val;
      ...
      strm.on('data', async data => {
        try {
          ...
          if (remove) {
            ...
            if (log && log.info) {
              log.info(`Completed "delete" on "${storeName}" for LevelDB key "${data.key}" (${isCompile ? 'compile' : 'render'}-time)`);
            }
