...
// TODO : ESM use... export class CachierDB extends Cachier {
  ...
  async registerPartials(partials, read, write) {
    ...
    if (read) {
      ...
      await execDB(null, opts, null, storage, true, false, ns.at.logger, function recordDB(storeName, key, params, json) {
  ...
  async read(name, forContent, extension, params) {
    ...
    await execDB(rwname, opts, null, storage, false, false, ns.at.logger, null, function openDB(storage) {
      ns.at.db = storage.db;
      ns.at.idb = storage.idb;
    });
  ...
  async write(name, data, forContent, extension, params) {
    ...
    const storage = { db: ns.at.db, idb: ns.at.idb };
    await execDB(rwname, opts, null, storage, false, put, ns.at.logger, null, function openDB(storage) {
      ns.at.db = storage.db;
      ns.at.idb = storage.idb;
    });
  }
  ...
  async clear(all = true) {
    ...
    await execDB(null, opts, null, storage, true, true, ns.at.logger, function recordDB(storeName, key, params, json) {
      ns.this.unregister(key);
    }, function openDB(storage) {
      ns.at.db = storage.db;
      ns.at.idb = storage.idb;
 ...
 */
async function execDB(name, optional, params, storage, close, valueOrRemove, logger, recordfunc, openFunc, openOnly) {
