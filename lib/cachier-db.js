...
// TODO : ESM use... export class CachierDB extends Cachier {
  ...
  async read(name, forContent, extension) {
    ...
    const ns = internal(this), id = ns.this.readWriteName(name, ns.at.options, forContent, extension);
    ...
    await execDB(id, ns.at.options, null, storage, false, ns.at.logger, null, function openDB(storage) {
      ns.at.db = storage.db;
      ns.at.idb = storage.idb;
    });
  ...
  async write(name, data, forContent, extension) {
    ...
    const storage = { db: ns.at.db, idb: ns.at.idb };
    await execDB(id, ns.at.options, null, storage, put, ns.at.logger, null, function openDB(storage) {
      ns.at.db = storage.db;
      ns.at.idb = storage.idb;
    });
  }
   ...
   */
  get readers() {
    const rdrs = super.readers, rdr = { read: execDB, finish: closeDB, readAllOnInit: true };
