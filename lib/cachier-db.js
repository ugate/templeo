...
// TODO : ESM use... export class CachierDB extends Cachier {
  ...
  async registerPartials(partials, read, write) {
    ...
    try {
      if (read) {
        ...
        await execDB(null, opts, null, storage, true, ns.at.logger, undefined, function recordDB(storeName, key, params, json) {
      ...
      if (read || write) {
        rtn = await rtn;
        await closeDB(storage, opts, ns.at.logger);
      }
    }
    return rtn;
  }
  ...
  async compile(name, template, extension) {
    ...
    try {
      ...
      await closeDB(storage, ns.this.options, ns.at.logger);
    }
    return fn;
  }
  ...
  async read(name, forContent, extension, params) {
    ...
    const ns = internal(this), opts = ns.this.options, rwname = ns.this.readWriteName(name, opts, params, forContent, extension);
    const cacheName = forContent ? 'partials' : 'sources';
    if (ns.at.logger.info) {
      ns.at.logger.info(`Reading template "${forContent ? 'partials' : 'sources'}" from DB @ "${rwname}"`);
    }
    const storage = { db: ns.at.db, idb: ns.at.idb }, rtn = { name };
    if (forContent) rtn.extension = extension;

    await execDB(rwname, opts, params, storage, false, ns.at.logger, undefined, null, function openDB(storage) {
      ns.at.db = storage.db;
      ns.at.idb = storage.idb;
    });
  ...
  async write(name, data, forContent, extension, params) {
    ...
    const ns = internal(this), opts = ns.this.options, rwname = ns.this.readWriteName(name, opts, params, forContent, extension);
    if (ns.at.logger.info) {
      ns.at.logger.info(`Writting template "${forContent ? 'partials' : 'sources'}" to DB @ "${rwname}"`);
    }
    ...
    const storage = { db: ns.at.db, idb: ns.at.idb };
    await execDB(rwname, opts, params, storage, false, ns.at.logger, put, null, function openDB(storage) {
      ns.at.db = storage.db;
      ns.at.idb = storage.idb;
    });
  }
  ...
  async clear(all = true) {
    ...
    const ns = internal(this), opts = ns.this.options, storage = { db: ns.at.db, idb: ns.at.idb };
    // remove all keys from the DB
    if (all && ns.at.logger.info) ns.at.logger.info(`Clearing all DB keys...`);
    await execDB(null, opts, null, storage, true, ns.at.logger, true, function recordDB(storeName, key, params, json) {
      ns.this.unregister(key);
    }, function openDB(storage) {
      ns.at.db = storage.db;
      ns.at.idb = storage.idb;
