...
async function execDB(name, optional, params, storage, formatter, close, log, isCompile, valueOrRemove, recordfunc, openFunc, openOnly) {
  ...
  let dataCnt;
  if (!name && (!policy.includes('read-all-on-init-when-empty') || (dataCnt = storage.data && Object.getOwnPropertyNames(storage.data).reduce((a, cnm) => cnm !== name ? a + 1 : a, 0)))) {
    if (log && log.info) {
      log.info(`DB: ${valueOrRemove === true ? 'Remove' : valueOrRemove ? 'Put' : 'Capture' } for all keys will be skipped for policy`
        + ` "${policy}"${dataCnt ? ` on ${dataCnt} non-empty records` : ''} (${isCompile ? 'compile' : 'render'}-time)`);
    }
    return;
  }
