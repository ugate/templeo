...
// TODO : ESM use... export class CachierDB extends Cachier {
   ...
   * @returns {Object|undefined} An object that contains the scan results (`undefined` when the `sourcePath` or `outputPath` and
      ...
      try {
        ...
        if (json.content) {
          if (!remove && rtn[names.partials][data.key] && ns.at.logger.warn) {
            ns.at.logger.warn(`The template partial for "${data.key}" in "options.partials" is overridden by the LevelDB scan`);
          }
          rtn[names.partials][data.key] = json;
        } else {
          if (!remove && rtn[names.code][data.key] && ns.at.logger.warn) {
            ns.at.logger.warn(`The template code for "${data.key}" in "options.partials" is overridden by the LevelDB scan`);
          }
