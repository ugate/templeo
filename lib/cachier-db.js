...
// TODO : ESM use... export class CachierDB extends Cachier {
  ...
  constructor(opts, formatFunc, logger) {
    super(opts instanceof TemplateDBOpts ? opts : new TemplateDBOpts(opts), formatFunc, true, logger);
  ...
  async registerPartials(partials, read, write) {
    ...
    try {
      if (read) {
        ...
        await execDB(null, opts, null, storage, true, logger, undefined, function recordDB(storeName, key, params, json) {
      ...
      if (read || write) {
        rtn = await rtn;
        await closeDB(storage, opts, logger);
      }
    }
    return rtn;
  }
  ...
  async compile(name, template, extension) {
    const ns = internal(this), logger = ns.this.logger;
    var fn;
    try {
      fn = await super.compile(name, template, extension);
    } finally {
      const storage = { db: ns.at.db, idb: ns.at.idb };
      await closeDB(storage, ns.this.options, logger);
    }
    return fn;
  }
  ...
  async read(name, forContent, extension, params) {
    ...
    if (logger.info) {
      logger.info(`Reading template "${forContent ? 'partials' : 'sources'}" from DB @ "${rwname}"`);
    }
    const storage = { db: ns.at.db, idb: ns.at.idb }, rtn = { name };
    if (forContent) rtn.extension = extension;

    await execDB(rwname, opts, params, storage, false, logger, undefined, null, function openDB(storage) {
      ns.at.db = storage.db;
      ns.at.idb = storage.idb;
    });
    if (storage[cacheName] && storage[cacheName][rwname]) {
      ...
      logger.debug(`${storage.idb ? 'IndexedDB' : 'LevelDB'} store = "${cacheName}" with "${name}" (full name: "${rwname}") NOT FOUND`);
    }
    return rtn;
  }
  ...
  async write(name, data, forContent, extension, params) {
    ...
    const rwname = ns.this.readWriteName(name, opts, params, ns.at, forContent, extension);
    if (logger.info) {
      logger.info(`Writting template "${forContent ? 'partials' : 'sources'}" to DB @ "${rwname}"`);
    }
    ...
    const storage = { db: ns.at.db, idb: ns.at.idb };
    await execDB(rwname, opts, params, storage, false, logger, put, null, function openDB(storage) {
      ns.at.db = storage.db;
      ns.at.idb = storage.idb;
    });
  }
  ...
  async clear(all = true) {
    ...
    const ns = internal(this), opts = ns.this.options, logger = ns.this.logger, storage = { db: ns.at.db, idb: ns.at.idb };
    // remove all keys from the DB
    if (all && logger.info) logger.info(`Clearing all DB keys...`);
    await execDB(null, opts, null, storage, true, logger, true, function recordDB(storeName, key, params, json) {
      ns.this.unregister(key);
    }, function openDB(storage) {
      ns.at.db = storage.db;
      ns.at.idb = storage.idb;
 ...
 */
async function closeDB(storage, optional, logger) {
  const db = storage.idb || storage.db;
  if (db) {
    await db.close();
