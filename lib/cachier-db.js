...
// TODO : ESM use... export class CachierDB extends Cachier {
  ...
  async register(data, read, write) {
    ...
    try {
      ...
      storage.idb = storage.idb || ns.at.idb;
    } finally {
...
async function dbWriter(name, path, ext, forContent, optional, params, store, data, writeFormatter, close, log, isCompile, recordFunc, openFunc) {
  ...
  if (log.info) {
    log.info(`DB: Writting template "${forContent ? 'data' : 'sources'}" for "${name}" @ "${path}" (${isCompile ? 'compile' : 'render'}-time)`);
  }
...
async function execDB(name, optional, params, storage, formatter, close, log, isCompile, valueOrRemove, recordfunc, openFunc, openOnly) {
  ...
  let dataCnt;
  if (!name && (!policy.includes('read-all-on-init-when-empty') || (dataCnt = storage.data && Object.getOwnPropertyNames(storage.data).length))) {
    if (log && log.info) {
      log.info(`DB: ${valueOrRemove === true ? 'Remove' : valueOrRemove ? 'Put' : 'Capture' } for all keys will be skipped for policy`
        + ` "${policy}"${dataCnt ? ` on ${dataCnt} non-empty records` : ''}`);
    }
    return;
  }
      ...
      if (close) {
        ...
        if (db && typeof db.close === 'function') {
          try {
            ...
            if (log && log.error) log.error(`DB: Failed to close connection (${isCompile ? 'compile' : 'render'}-time)`);
            return reject(err);
          }
        ...
        for (let storeName of storeNames) {
          ...
          req.onsuccess = () => {
            ...
            if (log && (log.info || log.debug)) {
              (log.debug || log.info)(`DB: Completed "${remove ? 'delete' : valType !== 'undefined' ? 'put' : 'get'}"`
        ...
        try {
          ...
          if (valType !== 'undefined') {
            ...
            if (log && (log.info || log.debug)) {
              ...
              (log.debug || log.info)(`DB: Completed "put" on "${storeName}" for LevelDB key "${name}"`
            ...
            } else if (log && log.info) log.info(`DB: Unable to "get" record for LevelDB key ${name}`);
          }
          if (val) {
          ...
          if (remove) {
            ...
            if (log) {
              if (storage[storeName][data.key] && log.warn){
                log.warn(`DB: The template  ${storeName} for "${data.key}" is overridden by LevelDB registration`);
...
async function closeDB(store, optional, log, isCompile) {
  ...
  store.db = store.idb = null; // need to clear the DB to prevent possible serialization of the DB store
}
