...
// TODO : ESM use... export class CachierDB extends Cachier {
  ...
  async read(name, forContent, extension) {
    ...
    if (ns.at.logger.info) {
      ns.at.logger.info(`Reading template "${forContent ? 'partials' : 'sources'}" for "${name}" (name) / "${id}" (ID)`);
    }
    const storage = { db: ns.at.db, idb: ns.at.idb }, rtn = { name, id };
    if (forContent) rtn.extension = extension;

    await execDB(name, ns.at.options, null, storage, false, ns.at.logger, null, function openDB(storage) {
      ns.at.db = storage.db;
      ns.at.idb = storage.idb;
    });
    if (storage[cacheName] && storage[cacheName][name]) {
      if (forContent) {
        rtn.content = storage[cacheName][name].content;
        rtn.extension = storage[cacheName][name].extension || rtn.extension;
        ns.this.registerPartial(rtn.name, rtn.content, rtn.extension);
      } else {
        try {
          if (storage[cacheName][name].func) rtn.func = Sandbox.deserialzeFunction(storage[cacheName][name].func, name, true);
        } catch (err) {
          err.message += ` <- ${storage.idb ? 'IndexedDB' : 'LevelDB'} store = "${cacheName}" with ID = "${id}" failed on read`
            + `deserialze of function: ${storage[cacheName][name].func}`;
          throw err;
        }
      }
    } else if (ns.at.logger.debug) {
      ns.at.logger.debug(`${storage.idb ? 'IndexedDB' : 'LevelDB'} store = "${cacheName}" with ID = "${id}" NOT FOUND`);
    }
    return rtn;
  }
  ...
  async write(name, data, forContent, extension) {
    ...
    const ns = internal(this), id = ns.this.readWriteName(name, ns.at.options, null, forContent, extension);
    if (ns.at.logger.info) {
      ns.at.logger.info(`Writting template "${forContent ? 'partials' : 'sources'}" for "${name}" (name) / "${id}" (ID)`);
    }
    ...
    const storage = { db: ns.at.db, idb: ns.at.idb };
    await execDB(name, ns.at.options, null, storage, put, ns.at.logger, null, function openDB(storage) {
      ns.at.db = storage.db;
      ns.at.idb = storage.idb;
    });
  }
  ...
  async clear(all = true) {
    ...
    if (db) {
      await db.close();
      if (ns.at.logger.info) ns.at.logger.info(`Closed DB connection(s)`);
    }
    if (all) {
      delete ns.at.db;
      delete ns.at.idb;
...
async function execDB(name, optional, params, storage, valueOrRemove, logger, recordfunc, openFunc, openOnly) {
  ...
  return new Promise(async (resolve, reject) => {
    if (!storage.db) {
      storage.db = (this && dbTypeName === 'indexedDB' && this[dbTypeName]) ||
        new Promise(async (resolveModule, rejectModule) => {
          if (logger && logger.info) logger.info(`Loading module "${dbTypeName}" passing location "${dbLocName}"`);
          var rslt = (typeof require !== 'undefined' ? require(dbTypeName) : /*await import(dbTypeName)*/null);
          rslt = rslt(dbLocName, (err, db) => err ? rejectModule(err) : resolveModule(db));
        ...
        try {
          ...
          if (valType !== 'undefined') {
            ...
            if (rcd) {
              try {
                val = JSON.parse(rcd);
                const hasContent = json.hasOwnProperty('content'), storeName = hasContent ? dbPStore : dbSStore;
                if (logger && (logger.info || logger.debug)) {
