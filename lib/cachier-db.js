...
// TODO : ESM use... export class CachierDB extends Cachier {
  ...
  constructor(opts, formatFunc, log) {
    super(opts instanceof TemplateDBOpts ? opts : new TemplateDBOpts(opts), formatFunc, true, log);
  ...
  async registerPartials(partials, read, write) {
    ...
    try {
      if (read) {
        ...
        await execDB(null, opts, null, storage, true, log, undefined, async function recordDB(storeName, key, params, json) {
  ...
  async read(name, forContent, extension, params) {
    ...
    if (log.info) {
      log.info(`Reading template "${forContent ? 'partials' : 'sources'}" from DB @ "${rwname}"`);
    }
    const storage = { db: ns.at.db, idb: ns.at.idb }, rtn = { name };
    if (forContent) rtn.extension = extension;

    await execDB(rwname, opts, params, storage, false, log, undefined, null, function openDB(storage) {
      ns.at.db = storage.db;
      ns.at.idb = storage.idb;
    });
    if (storage[cacheName] && storage[cacheName][rwname]) {
      if (forContent) {
        ...
        await ns.this.registerPartial(rtn.name, rtn.content, rtn.extension);
      } else {
        try {
      ...
      log.debug(`${storage.idb ? 'IndexedDB' : 'LevelDB'} store = "${cacheName}" with "${name}" (full name: "${rwname}") NOT FOUND`);
    }
    return rtn;
  }
  ...
  async write(name, data, forContent, extension, params) {
    ...
    const rwname = await ns.this.readWriteName(name, opts, params, ns.at, forContent, extension);
    if (log.info) {
      log.info(`Writting template "${forContent ? 'partials' : 'sources'}" to DB @ "${rwname}"`);
    }
    ...
    const storage = { db: ns.at.db, idb: ns.at.idb };
    await execDB(rwname, opts, params, storage, false, log, put, null, function openDB(storage) {
      ns.at.db = storage.db;
      ns.at.idb = storage.idb;
    });
  }
  ...
  async clear(all = true) {
    ...
    const ns = internal(this), opts = ns.this.options, log = ns.this.log, storage = { db: ns.at.db, idb: ns.at.idb };
    // remove all keys from the DB
    if (all && log.info) log.info(`Clearing all DB keys...`);
    await execDB(null, opts, null, storage, true, log, true, function recordDB(storeName, key, params, json) {
      ns.this.unregister(key);
    }, function openDB(storage) {
      ns.at.db = storage.db;
      ns.at.idb = storage.idb;
   ...
   */
  get operations() {
    const ops = super.operations, op = Object.freeze({ read: execDB, finish: closeDB });
 ...
 */
async function execDB(name, optional, params, storage, close, log, valueOrRemove, recordfunc, openFunc, openOnly) {
  const isOptionFunc = typeof optional === 'function';
  const policy = isOptionFunc ? optional('renderTimeReadPolicy') : optional.renderTimeReadPolicy;
  if (!name && (policy !== 'read-all-on-init-when-empty' || (storage.partials && Object.getOwnPropertyNames(storage.partials).length))) {
    if (log && log.debug) {
      log.debug(`DB ${valueOrRemove === true ? 'remove' : valueOrRemove ? 'put' : 'capture' } for all keys will be skipped for policy`
        + ` "${policy}"`);
    }
    return;
  }
  ...
  return new Promise(async (resolve, reject) => {
    const end = async error => {
      if (close) {
        ...
        } else if (log && log.info) log.info('DB may already be closed or manual "close" functionality is not implemented');
      }
    ...
    if (!storage.db) {
      storage.db = dbTypeName === 'indexedDB' ? this && this[dbTypeName] :
        new Promise(async (resolveModule, rejectModule) => {
          if (log && log.info) log.info(`Loading module "${dbTypeName}" passing location "${dbLocName}"${name ? ` (processing "${name}")` : ''}`);
          try {
            const initFunc = (typeof require !== 'undefined' ? require(dbTypeName) : /*await import(dbTypeName)*/null);
            const db = initFunc(dbLocName, (err, db) => {
              if (err) {
                const nerr = new err.constructor();
                nerr.stack = `Module "${dbTypeName}" failed to pass location "${dbLocName}"${name ? ` (processing "${name}")` : ''}\n${err.stack}`;
                rejectModule(nerr);
    ...
    const valType = typeof valueOrRemove, remove = valType === 'boolean' && valueOrRemove ? true : false, hasRecordfunc = typeof recordfunc === 'function';
    ...
    if (isIndexedDB) {
      ...
      if (!storage.db.isOpen()) {
        if (log && log.info) log.info(`Opening "${idbName}" from module "${dbTypeName}"`);
        try {
      ...
      if (name) { // single record processing
        ...
        return end();
      } else if (log && log.info) log.info(`${remove ? 'Removing' : 'Getting'} all LevelDB keys from ${storeNames.join()}`);
      // multi-record processing
 ...
 * @param {Object} [log] The log that can contain functions for each of the following: `error`/`warn`/`info`/`debug`.
 */
async function closeDB(storage, optional, log) {
  const db = storage.idb || storage.db;
  if (db) {
    await db.close();
    if (log && log.info) log.info(`Closed DB connection(s)`);
