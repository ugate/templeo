...
// TODO : ESM use... export class CachierDB extends Cachier {
  ...
  async read(name, forContent) {
    const ns = internal(this), id = ns.this.identity(name, forContent), cacheName = forContent ? ns.this.names.partials : ns.this.names.code;
      ...
      return new Promise((resolve, reject) => {
        ...
        req.onsuccess = event => {
          if (req.result) {
            ...
            else rtn.func = Cachier.deserialzeFunction(req.result.data, name);
            resolve(rtn);
    ...
    if (raw) {
      rtn = JSON.parse(raw);
      if (rtn.func) rtn.func = Cachier.deserialzeFunction(rtn.func, name);
    }
    return rtn;
  }
  ...
  async write(name, data, forContent) {
    ...
    if (!forContent) {
      ...
      else if (typeof put.func === 'string') put.func = Cachier.serialzeFunction(Cachier.deserialzeFunction(put.func, name));
    ...
    for (let snm of stores) {
      ...
      req.onsuccess = (cacheName => event => {
        ...
        try {
          ...
          else rtn[cacheName][cursor.key].func = Cachier.deserialzeFunction(req.result.data, rtn[cacheName][cursor.key].name);
          if (remove) {
