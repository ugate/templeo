...
// TODO : ESM use... export class CachierDB extends Cachier {
    ...
    if (idbName === 'IDBFactory') {
      ns.at.openIndexedDB = new Promise((resolve, reject) => {
        ...
        req.onupgradeneeded = event => {
          event.target.createObjectStore('partials', { autoIncrement: true });
          event.target.createObjectStore('sources', { autoIncrement: true });
        };
  ...
  async registerPartials(partials, read) {
    ...
    rtn.partials = rtn.partials || {};
    rtn.sources = rtn.sources || {};
  ...
  async clear(all = true) {
    const ns = internal(this), rtn = { partials: {}, sources: {} };
  ...
  return new Promise(async (resolve, reject) => {
    ...
    if (isIndexedDB) { // IndexedDB
      if (!storage.idb) {
        ...
        const stores = [dbPStore, dbSStore];
        ...
        for (let snm of stores) {
          const tx = storage.idb.transaction([snm]), store = tx.objectStore(snm), req = store.openCursor();
        ...
        try {
          ...
          if (remove) {
            ...
            if (storage[store][data.key] && logger && logger.warn) {
              logger.warn(`The template  ${store} for "${data.key}" in "options.partials" is overridden by LevelDB registration`);
            }
  ...
  return new Promise((resolve, reject) => {
    ...
    strm.on('data', async data => {
      try {
        ...
        if (json.content) {
          if (!remove && rtn.partials[data.key] && ns.at.logger.warn) {
