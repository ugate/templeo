...
// TODO : ESM use... export class CachierDB extends Cachier {
  ...
  async read(name, forContent, extension, params) {
    ...
    if (log.info) {
      log.info(`Reading template "${forContent ? 'partials' : 'sources'}" from DB @ "${path}"`);
    }
    const storage = { db: ns.at.db, idb: ns.at.idb }, rtn = { name };
    if (forContent) rtn.extension = extension;

    await execDB(path, opts, params, storage, formatter, false, log, true, undefined, null, function openDB(storage) {
      ns.at.db = storage.db;
      ns.at.idb = storage.idb;
    });
    if (storage[cacheName] && storage[cacheName][path]) {
      if (forContent) {
        rtn.content = storage[cacheName][path].content;
        rtn.extension = storage[cacheName][path].extension || rtn.extension;
        await ns.this.registerPartial(rtn.name, rtn.content, rtn.extension);
      } else {
        try {
          if (storage[cacheName][path].func) rtn.func = Sandbox.deserialzeFunction(storage[cacheName][path].func, name, true);
        } catch (err) {
          err.message += ` <- ${storage.idb ? 'IndexedDB' : 'LevelDB'} store = "${cacheName}" with "${name}" (full name: "${path}") failed on read`
            + `deserialze of function: ${storage[cacheName][path].func}`;
          throw err;
        }
      }
    } else if (log.debug) {
      log.debug(`${storage.idb ? 'IndexedDB' : 'LevelDB'} store = "${cacheName}" with "${name}" (full name: "${path}") NOT FOUND`);
    }
    return rtn;
  }
  ...
  async write(name, data, forContent, extension, params) {
    ...
    if (log.info) {
      log.info(`DB: Writting template "${forContent ? 'partials' : 'sources'}" to DB for "${name}" @ "${rwname}"`);
    }
    ...
    const storage = { db: ns.at.db, idb: ns.at.idb };
    await execDB(rwname, opts, params, storage, formatter, false, log, true, put, null, function openDB(storage) {
      ns.at.db = storage.db;
      ns.at.idb = storage.idb;
    });
  }
 ...
 */
async function dbReader(name, path, optional, params, store, readFormatter, close, log) {
  let rtn;
  try {
    rtn = await execDB(path, optional, params, store, readFormatter, close, log);
  } catch (err) {
    try {
