...
// TODO : ESM use... export class CachierDB extends Cachier {
  ...
  async registerPartials(partials, read, write) {
    ...
    try {
      if (read) {
        ...
        await execDB(null, opts, null, storage, ns.this.readFormatter, true, log, undefined, async function recordDB(storeName, key, params, json) {
  ...
  async read(name, forContent, extension, params) {
    ...
    const ns = internal(this), opts = ns.this.options, log = ns.this.log, formatter = ns.this.formatter;
    ...
    await execDB(rwname, opts, params, storage, formatter, false, log, undefined, null, function openDB(storage) {
      ns.at.db = storage.db;
      ns.at.idb = storage.idb;
    });
  ...
  async write(name, data, forContent, extension, params) {
    ...
    const ns = internal(this), opts = ns.this.options, log = ns.this.log, formatter = ns.this.writeFormatter;
    const rwname = await ns.this.readWriteName(name, opts, params, ns.at, forContent, extension);
    if (log.info) {
    ...
    const storage = { db: ns.at.db, idb: ns.at.idb };
    await execDB(rwname, opts, params, storage, formatter, false, log, put, null, function openDB(storage) {
      ns.at.db = storage.db;
      ns.at.idb = storage.idb;
    });
  }
  ...
  async clear(all = true) {
    ...
    await execDB(null, opts, null, store, null, true, log, true, function recordDB(storeName, key, params, json) {
      ns.this.unregister(key);
    }, function openDB(storage) {
      ns.at.db = storage.db;
      ns.at.idb = storage.idb;
 ...
 */
async function execDB(name, optional, params, storage, formatter, close, log, valueOrRemove, recordfunc, openFunc, openOnly) {
