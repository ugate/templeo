...
if (typeof process === 'undefined' || !process.versions || !process.versions.node || !process.release || process.release.name !== 'node') {
  throw new Error('TemplateFileOpts is only compatible with Node.js because it requires the Node.js file-system APIs.');
}
let Os, Path, Fs;
try {
  [Os, Path, Fs] = await Promise.all([import('node:os'), import('node:path'), import('node:fs')]);
} catch (error) {
  throw new Error('TemplateFileOpts is only compatible with Node.js because it requires the Node.js file-system APIs.', { cause: error });
}
