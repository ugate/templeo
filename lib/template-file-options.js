 ...
 * @property {Boolean} [watchPaths] When _true_, any `partialsPath` directories discovered when registering partials from a `read` operation
 ...
 * accordingly. The paths being watched are dependent upon `FSWatcher` support. See
 * [Node.js Fs.watch](https://nodejs.org/docs/latest/api/fs.html#fs_fs_watch_filename_options_listener) for more details. __NOTE:__
 * __When set to true at compile-time, all watched files will continue to be watched until `engine.clearCache` is called.__
 * __When set to true at render-time, all watched files will continue to be watched until the rendering function is called with `unwatchPaths` set to true.__
 * @property {Boolean} [unwatchPaths] When _true_, all currently `watchPaths` will be unwatched for changes when the rendering function is called.
 * __The rendering function call will return without actually rendering.__
