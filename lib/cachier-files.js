...
// TODO : ESM use... export class CachierFiles extends Cachier {
  ...
  async registerPartials(partials, read) {
    ...
    const pnm = ns.at.options.partialStoreName;
    const srtn = await super.registerPartials(partials, false), hasSuperPrtls = srtn && srtn[pnm] && srtn[pnm].length;
    var mkrtn;
    if (src && dest) {
    ...
    if (mkrtn && hasSuperPrtls) { // partials from file reads should always override any partials with the same name
      sloop:
      for (let prtl of srtn[pnm]) {
        for (let mprtl of mkrtn[pnm]) {
...
async function mkdirpMirror(ns, read, idir, odir, partialPrefix, cleanOutput = false, createOutput = true) {
  ...
  const pnm = ns.at.options.partialStoreName;
  const sdirs = await Fsp.readdir(idir), prtls = [], rtn = { [pnm]: [], dirs: [] }, logger = ns.at.logger;
  ...
  for (let sdir of sdirs) {
    ...
    if (stat.isDirectory()) {
      ...
      rtn[pnm] = rtnd[pnm] && rtnd[pnm].length ? rtn[pnm].concat(rtnd[pnm]) : rtn[pnm];
...
function clearModule(ns, path) {
  const snm = ns.at.options.sourceStoreName;
  if (ns.at.dbx[snm][path]) delete ns.at.dbx[snm][path];
