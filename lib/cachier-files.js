...
// TODO : ESM use... export class CachierFiles extends Cachier {
  ...
  async read(name, forContent, extension, params) {
    ...
    return readAndSet({ Os, Path, Fs, Fsp }, name, path, ns.this, forContent, extension, opts, ns.this, log);
  }
  ...
  async write(name, data, forContent, extension, params) {
    const ns = internal(this), opts = ns.this.options, log = ns.this.log;
    if (!opts.compiledPath || !opts.cacheRawTemplates) return;
    const path = await ns.this.readWriteName(name, opts, params, ns.at, forContent, extension);
    if (log.info && forContent) {
      if (log.info) log.info(`Skipping write on template partial for "${name}" (name)${extension ? ` "${extension}" (extension)` : ''}`
      + ` to file "${path}"${log.debug ? `:${Os.EOL}${data}` : ''}`);
      return; // partials are coming from the file system, no need to overwrite them
    }
    return writeAndSet({ Os, Path, Fs, Fsp }, name, path, data, ns.at, forContent, extension, opts, ns.this, log);
  }
 ...
 */
async function fileRenderReader(name, path, ext, optional, params, store, readFormatter, close, log) {
  const isOptionFunc = typeof optional === 'function';
  ...
  if (!path) {
    ...
    const readAll = optional('renderTimePolicy').includes('read-all-on-init-when-empty');
    if (!readAll && !optional('watchPartials') && Object.getOwnPropertyNames(store.partials).length) {
      if (log && log.debug) {
        log.debug(`Files cache initialization from source "${src}" to "${dest}" will be skipped for policy "${optional('renderTimePolicy')}"`);
      }
      return;
    }
  ...
  const rtn = await readAndRegisterPartial(mods, store, optional, reg, path, src, ext, log);
  return rtn.content;
}
 ...
 */
async function fileRenderWriter(name, path, ext, optional, params, store, data, writeFormatter, close, log) {
  const isOptionFunc = typeof optional === 'function';
  const src = isOptionFunc ? optional('partialsPath') : optional.partialsPath;
  const mods = await modules(optional);
  const extr = extractNameParts(mods, store, path, src);
  const registrant = { writeFormatter, modularize };
  const rtn = await writeAndSet(mods, extr.name, path, data, store, false, extr.extension || ext, optional, registrant, log);
  return rtn.func;
}
...
async function mkdirpMirror(mods, store, optional, registrant, read, idir, odir, partialPrefix, cleanOutput = false, createOutput = true, log = null, rendering = false) {
  ...
  for (let sdir of sdirs) {
    ...
    if (stat.isDirectory()) {
      ...
      if (read) {
        prtl = readAndRegisterPartial(mods, store, optional, registrant, indirx, partialPrefix, null, log);
        prtls.push(prtl);
      } else {
 ...
 */
function readAndRegisterPartial(mods, store, optional, registrant, path, partialPrefix, ext, log) {
  const { name, extension } = extractNameParts(mods, store, path, partialPrefix);
  return readAndSet(mods, name, path, store, true, extension || ext, optional, registrant, log);
}
 ...
 */
async function readAndSet(mods, name, path, store, forContent, extension, optional, registrant, log) {
  if (log && log.info) {
 ...
 */
async function writeAndSet(mods, name, path, data, store, forContent, extension, optional, registrant, log) {
  if (log && log.info) {
    log.info(`FILES: Writting template ${forContent ? 'partial' : 'code'} for "${name}" (name) from file "${path}"`);
  }
  const isOptionFunc = typeof optional === 'function';
  const encoding = isOptionFunc ? optional('encoding') : optional.encoding;
  ...
  if (dataType !== 'string') {
    if (log.debug) {
      log.debug(`Skipping write for unsupported type=${dataType} on template code for "${name}" (name)`
      + ` to file "${path}":${mods.Os.EOL}${data}`);
    }
    return;
  }
  if (!forContent) {
  ...
  await createPath(mods, store, path, log);
  await mods.Fsp.writeFile(path, data, !isOptionFunc ? optional : { encoding });
  const rtn = { name, path, extension };
  if (forContent) {
    rtn.content = data;
  } else {
    rtn.func = await modularize(path, true, optional, store);
  }
}
...
function watchPartialDir(mods, store, optional, registrant, dir, partialPrefix, log, rendering) {
  ...
  const partialWatchListener = async (type, filename) => {
    ...
    if (!stat && type === 'rename') { // file has been removed
      ...
      const prtl = await readAndRegisterPartial(mods, store, optional, registrant, path, partialPrefix, null, log);
 ...
 * @param {Object} [log] The log that can contain functions for each of the following: `error`/`warn`/`info`/`debug`
 */
async function createPath(mods, store, path, log) {
