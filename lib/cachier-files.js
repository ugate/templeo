...
// TODO : ESM use... export class CachierFiles extends Cachier {
  ...
  constructor(opts, formatFunc, logger) {
    ...
    if (ns.at.logger.info) {
      ns.at.logger.info(`Template destination set to: "${dest}"`);
    }
  }

  /**
   * Creates any missing directories/sub-directories within the {@link TemplateFileOpts} `outputPath` directory from `scanSourcePath`
   * sub-directories
   * @override
  ...
  async scan(registerPartial, unregisterPartial) {
    const ns = internal(this), src = ns.at.options.scanSourcePath, dest = ns.this.outputPath;
    ...
    if (src && dest) {
      ...
      ns.at.logger.info('File scan not performed since "scanSourcePath" and/or "outputPath" options have not been set'
        ...
        for (let mprtl of mkrtn.partials) {
          if (mprtl.name === prtl.name) {
            if (ns.at.logger.warn) {
              ns.at.logger.warn(`The template partial for "${prtl.name}" in "options.partials" is overridden by the file scan`);
            }
            continue sloop;
          }
        }
  ...
  async read(name, forContent) {
    const ns = internal(this), id = ns.this.identity(name, forContent), fpth = await pathify(ns, id, forContent);
    if (ns.at.logger.debug) {
      ns.at.logger.debug(`Reading template ${forContent ? 'partial' : 'code'} for "${name}" (name)/"${id}" (ID)`
        + ` from file "${fpth}"`);
    }
    const rtn = { name, id };
    if (forContent) {
      ...
      rtn.func = await ns.this.modularize(id);
    }
    return rtn;
  }
  ...
  async write(name, data, forContent) {
    ...
    const ns = internal(this), id = ns.this.identity(name, forContent), fpth = await pathify(ns, id, forContent, !forContent);
    if (ns.at.logger.debug) {
      ns.at.logger.debug(`${forContent ? 'Skipping write on' : 'Writting'} template ${forContent ? 'partial' : 'code'}`
        + ` for "${name}" (name)/"${id}" (ID) to file "${fpth}":${Os.EOL}${data}`);
    }
