...
// TODO : ESM use... export class CachierFiles extends Cachier {
  ...
  constructor(opts, formatFunc, logger) {
    super(opts instanceof TemplateFileOpts ? opts : new TemplateFileOpts(opts), formatFunc, false);
    const ns = internal(this), copts = ns.this.options, clogger = ns.this.logger;
    if (copts.watchRegistrationSourcePaths) ns.at.watchers = [];
    ns.at.createdDirs = copts.outputPathTempPrefix && copts.outputPath.includes(copts.outputPathTempPrefix) ?
    [Path.join(copts.outputPath, Path.sep)] : [];
    if (clogger.info) clogger.info(`Template destination set to: "${copts.outputPath}"`);
  }
  ...
  async registerPartials(partials, read, write) {
    const ns = internal(this), opts = ns.this.options, logger = ns.this.logger, src = ns.this.options.sourcePath, dest = opts.outputPath;
    ...
    if (src && dest) {
      ...
      mkrtn = mkdirpMirror(ns, opts, read, src, dest, prtlPrefix, true);
      if (hasSuperPrtls) mkrtn = await mkrtn;
      else return mkrtn;
    } else if (logger.info) {
      logger.info('Partials file read not performed since "sourcePath" and/or "outputPath" options have not been set'
      ...
      for (let prtl of srtn.partials) {
        for (let mprtl of mkrtn.partials) {
          if (mprtl.name === prtl.name) {
            ...
            if (logger.warn) {
              logger.warn(`The template partial for "${prtl.name}" is overridden by the file read`);
            }
            continue sloop;
          }
        }
  ...
  async read(name, forContent, extension, params) {
    ...
    const fpth = ns.this.readWriteName(name, opts, params, ns.at, forContent, extension);
    if (logger.debug) {
      logger.debug(`Reading template ${forContent ? 'partial' : 'code'} for "${name}" (name) from file "${fpth}"`);
    }
    const rtn = { name, extension };
    if (forContent) {
      rtn.content = (await Fsp.readFile(fpth, opts)).toString(opts.encoding);
  ...
  async write(name, data, forContent, extension, params) {
    ...
    if (logger.debug) {
      logger.debug(`${forContent ? 'Skipping write on' : 'Writting'} template ${forContent ? 'partial' : 'code'}`
    ...
    if (!writable && dataType !== 'string') {
      if (logger.debug) {
        logger.debug(`Skipping write for unsupported type=${dataType} on template ${forContent ? 'partial' : 'code'}`
  ...
  async clear(all = false) {
    ...
    if (all && opts.outputPathTempPrefix) {
      if (logger.info) {
        logger.info(`Clearing ALL temporary dirs that are prefixed with "${opts.outputPathTempPrefix}"`);
      }
      await cleanTempOutput(opts.outputPathTempPrefix);
    } else if (opts.outputPath) {
      if (logger.info) {
        logger.info(`Clearing "${opts.outputPath}"`);
      }
  ...
  async modularize(id, clear) {
    ...
    if (clear) clearModule(ns, opts, id);
 ...
 */
async function mkdirpMirror(ns, opts, read, idir, odir, partialPrefix, cleanOutput = false, createOutput = true) {
  if (cleanOutput) await rmrf(odir);
  const sdirs = await Fsp.readdir(idir), prtls = [], rtn = { partials: [], dirs: [] }, logger = ns.this.logger;
  if (cleanOutput || createOutput) {
    await mkdir(ns, odir);
    rtn.dirs.push(odir);
  }
  const watch = opts.watchRegistrationSourcePaths;
  if (watch) watchPartialDir(ns, idir, partialPrefix);
  let prtl;
  for (let sdir of sdirs) {
    const indirx = Path.join(idir, sdir), indir = Path.resolve(opts.templatePathBase, idir, sdir);
    ...
    if (stat.isDirectory()) {
      ...
      const rtnd = await mkdirpMirror(ns, opts, read, indirx, outdir, partialPrefix, false, false);
...
function readPartial(ns, dir, path, partialPrefix) {
  const { name, extension } = extractNameParts(ns, path, partialPrefix), logger = ns.this.logger;
...
function watchPartialDir(ns, dir, partialPrefix) {
  const files = {}, logger = ns.this.logger;
...
async function createPath(ns, path) {
  const fprts = Path.parse(path), logger = ns.this.logger;
  if (fprts.ext) fprts.base = fprts.ext = fprts.name = null;
  const fdir = Path.format(fprts);
  if (!ns.at.createdDirs.includes(fdir)) {
    if (logger.info) logger.info(`Creating template directory: ${fdir}`);
 ...
 */
function clearModule(ns, opts, path) {
  if (ns.at.dbx.sources[path]) delete ns.at.dbx.partials[path];
  if (!opts.useCommonJs) return;
...
async function readWriteName(name, optional, params, storage, forContent, extension, forContext, named) {
  ...
  const pths = [];
  const base = forContent ? opts.templatePathBase : opts.outputPath;
  const prtl = forContent && fullName.indexOf(opts.partialsPath) < 0 && opts.partialsPath;
