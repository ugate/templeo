...
// TODO : ESM use... export class CachierFiles extends Cachier {
  ...
  async registerPartials(partials, read, write) {
    const ns = internal(this), opts = ns.this.options, src = ns.this.options.sourcePath, dest = ns.this.outputPath;
  ...
  async read(name, forContent, extension, params) {
    const ns = internal(this), opts = ns.this.options, fpth = readWriteName(ns, opts, name, forContent, extension);
  ...
  /**
   * @override
   * @returns {TemplateFileOpts} The template compile options
   */
  get options() {
    const ns = internal(this);
  ...
  get outputPath() {
    const ns = internal(this), opts = ns.this.options;
    ns.at.outputPath = ns.at.outputPath || opts.outputPath;
    if (!ns.at.outputPath) {
 ...
 */
function readWriteName(ns, opts, name, forContent, extension) {
  const fpth = ns.this.readWriteName(name, opts, null, forContent, extension);
  if (Path.isAbsolute(fpth)) return fpth;
  const pths = [];
  const base = forContent ? opts.templatePathBase : ns.at.outputPath;
  const prtl = forContent && fpth.indexOf(opts.partialsPath) < 0 && opts.partialsPath;
