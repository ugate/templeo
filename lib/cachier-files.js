...
async function readWriteName(nmrs, name, optional, params, store, forContent, extension, forContext) {
  ...
  contentPath = forContent && path.replace(/\\/g, '/').indexOf(contentPath) < 0 && contentPath;
  let dir = '';
  if (!isBypass && !forContext && !forContext && !contentPath) {
  ...
  pths.push(path);console.log('DOH!!!!!!', Path.join(...pths), pathOpt, name, forTempl);
