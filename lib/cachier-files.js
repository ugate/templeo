...
// TODO : ESM use... export class CachierFiles extends Cachier {
  ...
  async register(data, read, write) {
    ...
    if (src && dest) {
      ...
      log.info('Partials file read not performed since "partialsPath" and/or "outputPath" options have not been set'
        ...
        for (let mdta of mkrtn.data) {
          if (mdta.name === dta.name) {
            ...
            if (log.warn) {
              ...
              log.warn(`The "options.${urld.optionName}" URL for "${dta.name}" is overridden by the file read`);
            }
            continue sloop;
          }
        }
  ...
  async write(name, data, forContent, extension, params) {
    ...
    if (log.info && forContent) {
      if (log.info) log.info(`Skipping write on template data for "${name}" (name)${extension ? ` "${extension}" (extension)` : ''}`
...
async function fileStart(store, optional, log, isCompile) {
  ...
  if (!Array.isArray(store.createdDirs)) {
    ...
    store.createdDirs = tprefix && opath && opath.replace(/\\/g, '/').includes(tprefix.replace(/\\/g, '/')) ? [opath + Path.sep] : [];
  }
...
async function fileRenderReader(name, path, ext, forContent, optional, params, store, readFormatter, close, log) {
  ...
  if (!path) {
    //const dname = isOptionFunc ? optional('defaultTemplateName') : optional.defaultTemplateName;
    if ((await fileStart(store, optional, log)) === true) return true;
    ...
    if (!readAll && !optional('watchPartials') && Object.getOwnPropertyNames(store.data).length) {
      if (log && log.debug) {
...
async function mkdirpMirror(mods, store, optional, registrant, read, idir, odir, partialPrefix, cleanOutput = false, createOutput = true, log = null, rendering = false) {
  ...
  for (let sdir of sdirs) {
    ...
    if (stat.isDirectory()) {
      ...
      if (read) {
        prtl = createReadAndSet(mods, true, store, optional, registrant, indirx, partialPrefix, null, log);
        prtls.push(prtl);
      } else {
  ...
  for (let prtl of prtls) {
    prtl = await prtl;
    rtn.data.push(prtl);
    if (log && log.info) log.info(`Registering template data "${prtl.name}"`);
 ...
 */
function createReadAndSet(mods, forContent, store, optional, registrant, path, partialPrefix, ext, log) {
  const { name, extension } = extractNameParts(mods, store, path, partialPrefix);
  return readAndSet(mods, name, path, store, forContent, extension || ext, optional, registrant, log);
}
...
async function writeAndSet(mods, name, path, data, store, forContent, extension, optional, registrant, log) {
  ...
  if (log && log.info) {
    log.info(`FILES: Writting template ${forContent ? 'partial' : 'code'} for "${name}" (name) to file "${writePath}"`);
  }
  const encoding = isOptionFunc ? optional('encoding') : optional.encoding;
  ...
  if (dataType !== 'string') {
    if (log.info) {
      log.info(`=> Skipping write for unsupported type=${dataType} on template code for "${name}" (name)`
      + ` to file "${writePath}" (forContent? ${forContent}):${log.debug ? ` ${mods.Os.EOL}${data}` : ''}`);
    }
    return;
  }
  if (!forContent) {
  ...
  await createPath(mods, store, writePath, log);
...
function watchPartialDir(mods, store, optional, registrant, dir, partialPrefix, log, rendering) {
  ...
  const partialWatchListener = async (type, filename) => {
    ...
    if (!stat && type === 'rename') { // file has been removed
      ...
      const prtl = await createReadAndSet(mods, true, store, optional, registrant, path, partialPrefix, null, log);
