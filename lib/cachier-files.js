...
async function fileRenderReader(name, path, ext, forContent, optional, params, store, readFormatter, close, log) {
  ...
  if (!path) {
    const unwatch = isOptionFunc ? optional('unwatchPartials') : optional.unwatchPartials;
    if (unwatch) {
      if (log && log.info) log.info('FS: 📁 File unwatch detected, skipping rendering');
  ...
  if (watch) {
    const prts = mods.Path.parse(path), dir = prts.dir.replace(/\\+|(?:\/\/+)/g, '/');
...
function watchPartialDir(mods, store, optional, registrant, dir, partialPrefix, log, isCompile) {
  const files = {};
  let watcher, logLabel = log && `(${isCompile ? 'compile' : 'render'}-time)`;
  const partialWatchListener = async (type, filename) => {
    ...
    if (!store.watchedDirs || !store.watchedDirs.includes(dir)) {
      if (log && log.info) {
        log.info(`FS: 👁️ Stopping watch directory ${dir}${store.watchedDirs ? ` in ${store.watchedDirs.join()}` : ''} from: "${type}" on ${path} ${logLabel}`);
      }
      watcher.close();
    }
    try {
