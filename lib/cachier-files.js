...
// TODO : ESM use... export class CachierFiles extends Cachier {
  ...
  /**
   * Reads either template content or template code from the internal file system
   * @override
  ...
  async read(name, forContent) {
    const ns = internal(this), fpth = readWriteName(ns, name, forContent);
  ...
  /**
   * Writes either template content or template code to the internal file system
   * @override
   * @param {String} name The template name that uniquely identifies the template content
 ...
 */
function readWriteName(ns, name, forContent) {
  const fpth = ns.this.readWriteName(name, forContent);
