...
// TODO : ESM use... export class CachierFiles extends Cachier {
  ...
  async register(data, read, write) {
    ...
    if (src && dest) {
      ...
      log.info('FS: Partials read not performed since "partialsPath" and/or "outputPath" options have not been set'
        ...
        for (let mdta of mkrtn.data) {
          if (mdta.name === dta.name) {
            ...
            if (log.warn) {
              ...
              log.warn(`FS: The "options.${urld.optionName}" URL for "${dta.name}" is overridden by the file read`);
            }
            continue sloop;
          }
        }
  ...
  async write(name, data, forContent, extension, params) {
    ...
    if (log.info && forContent) {
      if (log.info) log.info(`FS: Skipping write on template data for "${name}" (name)${extension ? ` "${extension}" (extension)` : ''}`
  ...
  async clear(all = false) {
    const ns = internal(this), opts = ns.this.options, log = ns.this.log;
    unwatchers(ns.at.watchers, ns.at.watchedDirs, log);
 ...
 * to indicate no further rendering should take place (e.g. )
 */
async function fileRenderReader(name, path, ext, forContent, optional, params, store, readFormatter, close, log) {
  ...
  if (!path) {
    ...
    if (unwatch) {
      if (log && log.info) {
        log.info('FS: File unwatch detected, skipping rendering');
      }
      unwatchers(store.watchers, store.watchedDirs, log);
      return true;
    }
    if ((await fileStart(store, optional, log)) === true) return;
    ...
    if (!readAll && !(isOptionFunc ? optional('watchPartials') : optional.watchPartials) && Object.getOwnPropertyNames(store.data).length) {
      if (log && log.debug) {
        log.debug(`FS: Cache initialization from source "${src}" to "${dest}" will be skipped for policy "${optional('renderTimePolicy')}"`);
      }
      return;
    }
    await mkdirpMirror(mods, store, optional, reg, readAll, src, dest, src, true, true, log);
    return;
  }
...
async function mkdirpMirror(mods, store, optional, registrant, read, idir, odir, partialPrefix, cleanOutput = false, createOutput = true, log = null, isCompile = false) {
  ...
  for (let prtl of prtls) {
    prtl = await prtl;
    rtn.data.push(prtl);
    if (log && log.info) log.info(`FS: Registering template data "${prtl.name}"`);
...
async function writeAndSet(mods, name, path, data, store, forContent, extension, optional, registrant, log, isCompile) {
  ...
  if (!useCache) {
    if (log && log.info) {
      log.info(`FS: Skipping write due to "options.cacheRawTemplates" is turned off for template "${name}" @ "${path}"${logLabel}`);
    }
    return;
  }
  ...
  if (log && log.info) {
    log.info(`FS: Writting template ${forContent ? 'partial' : 'code'} for "${name}" (name) to file "${writePath}"${logLabel}`);
  }
  const encoding = isOptionFunc ? optional('encoding') : optional.encoding;
  ...
  if (dataType !== 'string') {
    if (log.info) {
      log.info(`FS: Skipping write for unsupported type=${dataType} on template code for "${name}" (name)`
...
function watchPartialDir(mods, store, optional, registrant, dir, partialPrefix, log, isCompile) {
  ...
  const partialWatchListener = async (type, filename) => {
    ...
    if (!store.watchedDirs || !store.watchedDirs.includes(dir)) {
      if (log && log.info) {
        log.info(`FS: Stopping watch directory ${dir}${store.watchedDirs ? ` in ${store.watchedDirs.join()}` : ''} (from: "${type}" on ${path})`);
      }
      watcher.close();
    }
    try {
    ...
    if (!stat && type === 'rename') { // file has been removed
      ...
      if (log && log.info) {
        ...
        log.info(`FS: Unregistering watched "${type}" template partial "${name}"${extTxt} from: ${path}`);
      }
      ...
      if (log && log.info) log.info(`FS: Registered watched "${type}" template partial "${prtl.name}" from: ${path}`);
    }
  };
  watcher = mods.Fs.watch(dir, { persistent: false }, partialWatchListener);
  store.watchers.push({ dir, watcher });
  store.watchedDirs.push(dir);
  if (log && log.info) log.info(`FS: Watching template partial directory "${dir}"`);
 ...
 * @param {Object} [log] The log that can contain functions for each of the following: `error`/`warn`/`info`/`debug`
 */
function unwatchers(watchers, watchedDirs, log) {
  let cnt = 0;
  if (watchers) {
  ...
  if (log && log.info) log.info(`FS: Closed ${cnt} file watchers`);
}
