...
async function fileStart(store, optional, log, isCompile) {
  ...
  if (log && log.info) log.info(`FS: Started cache with template destination set to: "${opath}" (${isCompile ? 'compile' : 'render'}-time)`);
}
...
async function createPath(mods, store, path, log, isCompile) {
  ...
  if (!store.createdDirs.includes(fdir)) {
    if (log && log.info) log.info(`FS: Creating directory path: ${fdir} (${isCompile ? 'compile' : 'render'}-time)`);
