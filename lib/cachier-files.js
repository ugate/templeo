...
// TODO : ESM use... export class CachierFiles extends Cachier {
  ...
  async write(name, data, forContent, extension, params) {
    ...
    // TODO : move start and create path to proper location
    await fileStart(ns.at, opts, log, true);
    await createPath({ Os, Path, Fs, Fsp }, ns.at, path);

    return writeAndSet({ Os, Path, Fs, Fsp }, name, path, data, ns.this, forContent, extension, opts, ns.this, log);
  }
   ...
   * @inheritdoc
   */
  async modularize(path, clear) {
    const ns = internal(this), opts = ns.this.options;
    return modularize(path, clear, opts, ns.at);
  }
 ...
 */
async function fileRenderReader(name, optional, params, store, readFormatter, close, log) {
  const isOptionFunc = typeof optional === 'function';
 ...
 * @private
 * @ignore
 * @param {String} path The name of template that will be read
 * @param {(TemplateDBOpts | Function)} optional Either the options or a `function(name:String):*` that returns an
 * option value by name
 * @param {URLSearchParams} [params] The search parameters to use for the read 
 * @param {Object} store The JSON storage space
 ...
 */
async function fileRenderWriter(path, optional, params, store, data, writeFormatter, close, log) {
  const isOptionFunc = typeof optional === 'function';
  const src = isOptionFunc ? optional('partialsPath') : optional.partialsPath;
  const mods = await modules(optional);
  const { name, extension } = extractNameParts(mods, store, path, src);
  const rtn = await writeAndSet(mods, name, path, data, store, false, extension, optional, { writeFormatter, modularize }, log);
  return rtn.func;
}
...
function readAndRegisterPartial(mods, store, optional, registrant, path, partialPrefix, log) {
  ...
  return readAndSet(mods, name, path, store, true, extension, optional, registrant, log);
}
...
async function readAndSet(mods, name, path, store, forContent, extension, optional, registrant, log) {
  ...
  if (forContent) {
    ...
    rtn.func = await registrant.modularize(path, true, optional, store);
  }
  return rtn;
}
...
async function writeAndSet(mods, name, path, data, store, forContent, extension, optional, registrant, log) {
  ...
  const isOptionFunc = typeof optional === 'function';
  const encoding = isOptionFunc ? optional('encoding') : optional.encoding;
  data = typeof data === 'function' ? data.toString() : data;
