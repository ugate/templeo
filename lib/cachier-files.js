...
if (typeof process === 'undefined' || !process.versions || !process.versions.node || !process.release || process.release.name !== 'node') {
  throw new Error('CachierFiles is only compatible with Node.js because it requires the Node.js file-system APIs.');
}
let TemplateFileOpts, Os, Fs, Path;
try {
  [{ default: TemplateFileOpts }, Os, Fs, Path] = await Promise.all([
    import('./template-file-options.js'),
    import('node:os'),
    import('node:fs'),
    import('node:path')
  ]);
} catch (error) {
  throw new Error('CachierFiles is only compatible with Node.js because it requires the Node.js file-system APIs.', { cause: error });
}
...
async function fileRenderWriter(name, path, ext, forContent, optional, params, store, data, writeFormatter, close, log) {
  ...
  if (!forContent) {
    ...
    const outputPath = (isOptionFunc ? optional('outputPath') : optional.outputPath) || '';
    const outputBase = mods.Path.isAbsolute(outputPath) ? mods.Path.normalize(outputPath) : mods.Path.resolve(relativeTo, outputPath);
    const outputPrefix = `${outputBase}${mods.Path.sep}`;
    const candidate = mods.Path.isAbsolute(path) ? mods.Path.normalize(path) : mods.Path.resolve(relativeTo, path);
    const defaultTemplateName = isOptionFunc ? optional('defaultTemplateName') : optional.defaultTemplateName;
    if (candidate === outputBase || candidate.startsWith(outputPrefix)) {
      writePath = candidate;
    } else if (name === defaultTemplateName) {
      const sourceExtension = extr.extension || (ext ? `${/^\./.test(ext) ? '' : '.'}${ext}` : '');
      writePath = mods.Path.join(outputBase, `${name}${sourceExtension}`);
    } else {
      const relativeBase = mods.Path.resolve(relativeTo);
      let relativePath = mods.Path.relative(relativeBase, candidate);
      if (!relativePath || relativePath === '..' || relativePath.startsWith(`..${mods.Path.sep}`) || mods.Path.isAbsolute(relativePath)) {
        relativePath = mods.Path.basename(candidate);
      }
      writePath = mods.Path.join(outputBase, relativePath);
    }
  }
  const registrant = { writeFormatter, modularize };
  const rtn = await writeAndSet(mods, extr.name, writePath, data, store, forContent, extr.extension || ext, optional, registrant, log);
  return rtn && rtn.func;
}
...
async function writeAndSet(mods, name, path, data, store, forContent, extension, optional, registrant, log, isCompile) {
  ...
  return withPending(store, 'writes', pendingKey, async () => {
    ...
    const opath = (isOptionFunc ? optional('outputPath') : optional.outputPath) || '';
    const relativeBase = mods.Path.resolve(relativeTo);
    const outputBase = mods.Path.isAbsolute(opath) ? mods.Path.normalize(opath) : mods.Path.resolve(relativeBase, opath);
    const candidate = mods.Path.isAbsolute(path) ? mods.Path.normalize(path) : mods.Path.resolve(relativeBase, path);
    const outputPrefix = `${outputBase}${mods.Path.sep}`;
    let writePath;
    if (candidate === outputBase || candidate.startsWith(outputPrefix)) {
      writePath = candidate;
...
function extractNameParts(mods, store, path, partialPrefix) {
  ...
  if (ppx) {
    const prefixIndex = name.indexOf(ppx);
    if (prefixIndex >= 0) name = name.substring(prefixIndex + ppx.length).replace(/^\\|\//, '');
  }
