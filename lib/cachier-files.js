...
// TODO : ESM use... export class CachierFiles extends Cachier {
  ...
  constructor(opts, formatFunc, logger) {
    ...
    if (!(opts instanceof TemplateFileOpts)) throw new Error(`Options must be of type ${TemplateFileOpts.constructor.name} not ${opts}`);
  ...
  /**
   * Creates any missing directories/sub-directories within the {@link TemplateFileOpts} `outputPath` directory from `sourcePath`
   * sub-directories
   * @override
   ...
   * 
   * The returned registration results object contains the following properties:
  ...
  async registerPartials(partials, read) {
    ...
    const srtn = await super.registerPartials(partials, read);
    var mkrtn;
    if (src && dest) {
      const prtlPrefix = ns.at.options.partialsPath || '';
      mkrtn = mkdirpMirror(ns, src, dest, prtlPrefix, true);
    ...
    if (mkrtn && srtn) { // partials from file reads should always override any partials with the same name
      sloop:
  ...
  async read(name, forContent) {
    const ns = internal(this), fpth = ns.this.fullName(name, forContent);
  ...
  fullName(name, forContent) {
    const ns = internal(this);
    const fpth = super.fullName(name, forContent);
 ...
 */
async function mkdirpMirror(ns, idir, odir, partialPrefix, cleanOutput = false, createOutput = true) {
  ...
  for (let sdir of sdirs) {
    ...
    if (stat.isDirectory()) {
      ...
      const rtnd = await mkdirpMirror(ns, indirx, outdir, partialPrefix, false, false);
    ...
    } else if (stat.isFile()) prtls.push(readPartial(ns, indir, indirx, partialPrefix));
  }
...
function readPartial(ns, dir, path, partialPrefix) {
  const name = extractPartialName(ns, path, partialPrefix), logger = ns.at.logger;
...
function watchPartialDir(ns, dir, partialPrefix) {
  ...
  const partialWatchListener = async (type, filename) => {
    ...
    if (!stat && type === 'rename') {
      ...
      ns.this.unregisterPartial(name);
    } else if (stat) {
...
function clearModule(ns, path) {
  if (ns.at.dbx[ns.at.names.rndrs][path]) delete ns.at.dbx[ns.at.names.rndrs][path];
