  ...
  async write(name, data, forContent, extension, params) {
    ...
    if (log.info) {
      log.info(`${forContent ? 'Skipping write on' : 'Writting'} template ${forContent ? 'partial' : 'code'}`
        + ` for "${name}" (name)${extension ? ` "${extension}" (extension)` : ''} to file "${path}"${log.debug ? `:${Os.EOL}${data}` : ''}`);
    }
 ...
 */
async function renderReader(name, optional, params, store, readFormatter, close, log) {
  const isOptionFunc = typeof optional === 'function';
 ...
 */
async function renderWriter(path, optional, params, store, data, writeFormatter, close, log) {
  const isOptionFunc = typeof optional === 'function';
  const src = isOptionFunc ? optional('partialsPath') : optional.partialsPath;
  const mods = await modules(optional);
