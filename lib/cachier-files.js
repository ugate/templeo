...
async function fileStart(storage, optional, log, isCompile) {
  ...
  const unwatch = isOptionFunc ? optional('unwatch') : optional.unwatch;
  const cjs = isOptionFunc ? optional('useCommonJs') : optional.useCommonJs;
  const Path = cjs ? require('path') : /*await import('path')*/null;
  const watch = isOptionFunc ? optional('watchRegistrationPartialPaths') : optional.watchRegistrationPartialPaths;
  ...
  storage.createdDirs = tprefix && opath && opath.replace(/\\/g, '/').includes(tprefix.replace(/\\/g, '/')) ? [Path.join(opath, Path.sep)] : [];
...
async function fileReader(name, optional, params, storage, close, log) {
  ...
  if (!name) {
    ...
    try {
      await mkdirpMirror(mods, storage, optional, reg, true, src, dest, src, true, true, log);
    } finally {
      storage.watchers
    }
  }
  const rtn = await readAndRegisterPartial(mods, storage, optional, reg, name, src, log);
  return rtn.content;
}
...
async function mkdirpMirror(mods, storage, optional, registrant, read, idir, odir, partialPrefix, cleanOutput = false, createOutput = true, log = null) {
  ...
  const sdirs = await Fsp.readdir(idirResolved), prtls = [], rtn = { partials: [], dirs: [] };
  if (cleanOutput || createOutput) {
    await mkdir(mods, storage, odir, log);
    rtn.dirs.push(odir);
  }
  const watch = isOptionFunc ? optional('watchRegistrationPartialPaths') : optional.watchRegistrationPartialPaths;
  if (watch) watchPartialDir(mods, storage, optional, registrant, idirResolved, partialPrefix, log);
  let prtl;
...
function watchPartialDir(mods, storage, optional, registrant, dir, partialPrefix, log) {
  ...
  const partialWatchListener = async (type, filename) => {
    ...
    if (!stat && type === 'rename') { // file has been removed
      ...
      const meta = typeof optional === 'function' ? optional('compiledMetaFilename') : optional.compiledMetaFilename;
      //mods.Path.resolve(tbase, idir, sdir);
  ...
  if (storage.watchers) storage.watchers.push({ dir, watcher });
