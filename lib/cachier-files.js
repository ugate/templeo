...
class CachierFiles extends Cachier {
  ...
  async clear(all = false) {
    const ns = internal(this), opts = ns.this.options, log = ns.this.log;
    unwatchers(ns.at.watchers, ns.at.watchedDirs, ns.at.watchedFiles, log, true);
...
async function fileRenderReader(name, path, ext, forContent, optional, params, store, readFormatter, close, log) {
  ...
  if (!path) {
    ...
    if (unwatch) {
      ...
      if (watchCnt) unwatchers(store.watchers, store.watchedDirs, store.watchedFiles, log);
      return true;
    }
 ...
 */
function trackWatchedFile(mods, store, path, partial) {
  if (!store.watchedFiles || typeof store.watchedFiles !== 'object') store.watchedFiles = {};
  const resolved = mods.Path.resolve(path);
  ...
  async function scan(current) {
    ...
    for (const entry of entries) {
      ...
      if (entry.isDirectory()) {
        ...
        trackWatchedFile(mods, store, path, partial);
        previous.delete(path);
        count++;
      }
    }
...
function watchPartialDir(mods, store, optional, registrant, dir, partialPrefix, log, isCompile) {
  ...
  const filesCachierWatchListener = (type, filename) => {
    ...
    pending.set(key, setTimeout(() => {
      pending.delete(key);
      processWatchEvent(type, filename).catch(error => {
        if (log && log.error) log.error(error);
      });
    }, 50));
  };
  watcher = mods.Fs.watch(resolvedDir, { persistent: false }, filesCachierWatchListener);
  watcher.on('error', error => {
    if (log && log.error) log.error(error);
  });
  store.watchedDirs.push(resolvedDir);
  store.watchers.push(Object.freeze({ dir: resolvedDir, isCompile, watcher }));
  if (log && log.info) {
 ...
 */
function unwatchers(watchers, watchedDirs, watchedFiles, log, isCompile) {
  let count = 0;
  if (watchers) {
...
async function modularize(path, clear, optional, store) {
  ...
  let specifier = /^[a-zA-Z][a-zA-Z\d+.-]*:/.test(path) ? path : Url.pathToFileURL(path).href;
  if (clear) specifier += `${specifier.includes('?') ? '&' : '?'}templeo=${Date.now()}`;
