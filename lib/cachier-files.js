...
// TODO : ESM use... export class CachierFiles extends Cachier {
  ...
  async registerPartials(partials, read) {
    ...
    const srtn = await super.registerPartials(partials, false), hasSuperPrtls = srtn && srtn.partials && srtn.partials.length;
    var mkrtn;
    if (src && dest) {
  ...
  for (let sdir of sdirs) {
    ...
    if (stat.isDirectory()) {
      ...
      if (read) {
        ...
        prtl = { name: extractPartialName(ns, indirx, partialPrefix), path: indirx };
        prtl.read = false;
...
function readWriteName(ns, name, forContent) {
  const fpth = ns.this.readWriteName(name, ns.at.options, forContent);
