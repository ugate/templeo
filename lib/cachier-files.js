...
// TODO : ESM use... export class CachierFiles extends Cachier {
  ...
  async generateCode(name, path, code, write = true, store = true) {
    const ns = Cachier.internal(this);
    const rtn = await super.generateCode(name, path, code, false, store); // never write since the coded is written to file
    rtn.coded = `${ns.at.options.useCommonJs ? 'module.exports=' : 'export '}${rtn.coded}`;
    if (write) await ns.this.write(name, Path.join(ns.at.outputPath, path), rtn.coded);
    return rtn;
  }
  ...
  async read(name, path, forContent) {
    const ns = Cachier.internal(this), base = ns.at.options.relativeTo;
    const fpth = Path.isAbsolute(path) || path.startsWith(base) ? path : Path.join(base, path);
    const rtn = { name, path }, data = await Fs.readFile(fpth, ns.at.options);
...
async function mkdirpMirror(ns, idir, odir, registerPartial, partialPrefix, cleanOutput = false, createOutput = true) {
  ...
  for (let sdir of sdirs) {
    const indirx = Path.join(idir, sdir), indir = Path.resolve(ns.at.options.relativeTo, idir, sdir), stat = await Fs.stat(indir);
    if (stat.isDirectory()) {
