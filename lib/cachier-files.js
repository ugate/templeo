 ...
 */
async function fileRenderReader(name, path, optional, params, store, readFormatter, close, log) {
  const isOptionFunc = typeof optional === 'function';
  ...
  const rtn = await readAndRegisterPartial(mods, store, optional, reg, path, src, log);
  return rtn.content;
}
 ...
 */
async function fileRenderWriter(name, path, optional, params, store, data, writeFormatter, close, log) {
  const isOptionFunc = typeof optional === 'function';
  const src = isOptionFunc ? optional('partialsPath') : optional.partialsPath;
  const mods = await modules(optional);
  const extr = extractNameParts(mods, store, path, src);
  const rtn = await writeAndSet(mods, extr.name, path, data, store, false, extr.extension, optional, { writeFormatter, modularize }, log);
  return rtn.func;
}
