...
// TODO : ESM use... export class CachierFiles extends Cachier {
  ...
  constructor(opts, formatFunc) {
    ...
    if (!ns.at.options.pathScanDest) ns.at.outputTempPrefix = TMP_PREFIX;
  ...
  async scan(registerPartial, unregisterPartial) {
    ...
    const srtn = await super.scan(registerPartial, unregisterPartial);
    var mkrtn;
    if (src && dest) {
  ...
  async clear(all = false) {
    ...
    if (ns.at.pathScanDest) {
      if (ns.at.options.logger.info) {
        ns.at.options.logger.info(`Clearing "${ns.at.pathScanDest}"`);
      }
      await rmrf(ns.at.pathScanDest);
      ns.at.pathScanDest = ns.at.options.pathScanDest || await Fsp.mkdtemp(Path.join(Os.tmpdir(), ns.at.outputTempPrefix));
    }
  }
}
