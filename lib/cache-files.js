...
// TODO : ESM use... export class CacheFiles {
   ...
   */
  async setup() {
    const ns = Cache.internal(this), src = ns.options.outputSourcePath, dest = ns.options.outputPath;
  ...
  async getTemplate(path) {
    ...
    var code = await Fs.readFile(path, ns.options);
    code = code && ns.isFormatOnRead && ns.formatter ? ns.formatter(code, ns.options.outputFormatting) : code;
  ...
  async generateTemplate(name, path, code, write = true) {
    ...
    if (write) {
      ...
      await Fs.writeFile(path, rtn.code, ns.options);
  ...
  async readPartial(path) {
    const ns = Cache.internal(this);
    return Fs.readFile(path, ns.options);
  }
  ...
  async writePartial(name, path, data) {
    const ns = Cache.internal(this);
    return Fs.writeFile(path, data, ns.options);
  }
