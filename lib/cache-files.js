...
// TODO : ESM use... export class CacheFiles {
  ...
  constructor(opts, formatFunc) {
    ...
    ns.at.outputPath = ns.at.options.outputPath || Os.tmpdir();
  }
  ...
  async setup(registerPartial) {
    const ns = Cache.internal(this), src = ns.at.options.outputSourcePath, dest = ns.at.outputPath;
    var dirs = null;console.log(ns.at.options, dest)
    if (src && dest) {
      dirs = mkdirpMirror(src, dest, true);
      //registerPartial
    }
    return dirs;
  }
  ...
  async generateTemplate(name, path, code, write = true) {
    ...
    if (write) {
      ...
      await Fs.writeFile(path, rtn.coded, ns.at.options);
