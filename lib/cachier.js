...
class Cachier {
  ...
  async unregister(name) {
    const ns = internal(this);
    cacheRemove(ns.at, 'data', name);
    cacheRemove(ns.at, 'sources', name);
  }
...
async function readWriteName(nmrs, name, optional, params, store, forContent, extension, forContext) {
  ...
  if (extRx.test(url.pathname)) {
    ...
    ext = isOptionFunc ? optional('useCommonJs') : optional.useCommonJs ? 'cjs' : 'mjs';
  }
  if (ext) {
  ...
  return `${base}${realName}${query ? `?${query}` : ''}`;
}
