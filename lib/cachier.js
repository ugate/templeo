...
// TODO : ESM use... export class Cachier {
  ...
  async write(name, data, forContent) {
    ...
    data = data || '';
    const ns = Cachier.internal(this), id = ns.this.identity(name, forContent), cacheName = forContent ? ns.at.names.partials : ns.at.names.code;
    ...
    if (url) await fetcher(ns, `${url}/${id}`, data, rejUnauth);
