...
// TODO : ESM use... export class Cachier {
  ...
  registerPartial(name, contentOrParams, extension) {
    ...
    const rwname = ns.this.readWriteName(name, opts, params, ns.at, true, extension);
  ...
  async compile(name, template, params, extension) {
    const ns = internal(this), opts = ns.this.options, logger = ns.this.logger;
    ...
    if (!hasTmpl) { // partial template includes
      ...
      cnm = ns.this.readWriteName(name, opts, params, ns.at, true, extension);
      if (!opts.cacheRawTemplates || !ns.at.dbx.partials[cnm] || !ns.at.dbx.partials[cnm].hasOwnProperty('content')) {
        ...
        ns.this.registerPartial(cnm, content);
      } else content = ns.at.dbx.partials[cnm].content;
      // read/load rendering function?
      const snm = ns.this.readWriteName(name, opts, params, ns.at, false);
    ...
    if (!func) { // no rendering function requires a fresh compile
      func = Sandbox.compile(name, content, ns.at.dbx.partials, ns.at.dbx.sources, opts, ns.this.readWriteNames,
          ns.this.readers, ns.at.dbx.helpers, logger);
      if (logger.debug) logger.debug(`Created sandbox for: ${Sandbox.serialzeFunction(func)}`);
      if (func) {
        if (logger.info) logger.info(`Compiled template named "${func.name}"${cnm ? ` @ "${cnm}"` : ''}`);
  ...
  async read(name, forContent, extension, params) {
    const ns = internal(this), opts = ns.this.options, logger = ns.this.logger, cacheName = forContent ? 'partials' : 'sources';
    const isFetch = forContent && isAbsName(name, opts);
    const rwname = ns.this.readWriteName(name, opts, params, ns.at, forContent, extension);
    if (logger.info) {
      logger.info(`Reading template ${forContent ? 'partial' : 'code'} `
  ...
  async write(name, data, forContent, extension, params) {
    ...
    if (logger.info) {
      logger.info(`Writting template ${forContent ? 'partial' : 'code'} ${isFetch ? `POST "${rwname}"` : 'for "${rwname}" to memory'}`);
    }
   ...
   */
  readWriteName(partialName, optional, params, storage, forContent, extension, forContext) {
    let namers = this.readWriteNames, named;
    if (!Array.isArray(namers)) namers = [namers];
    for (let namer of namers) {
      named = namer(partialName, optional, params, storage, forContent, extension, forContext, named);
    }
    return named;
  }
   ...
   * @returns {(Function | Function[])} One or more functions responsible for formatting template names into a full path name
 ...
 * @param {Object} [log] The logger that can contain functions for each of the following: `error`/`warn`/`info`/`debug`.
 ...
 */
function readWriteName(name, optional, params, storage, forContent, extension, forContext) {
  const isOptionFunc = typeof optional === 'function';
