...
// TODO : ESM use... export class Cachier {
   ...
   * @param {String} name The module ID to load (with or w/o the file extension)
  ...
  async modularize(name, clear) {
    ...
    return modularize(name, store, clear);
  }
 ...
 */
async function defaultReader(name, path, optional, params, store, readFormatter, close, log) {
  if (!path) return;
  const mods = await modules(optional), reg = initRegistrant(path, optional, store, readFormatter);
  return readAndSet(mods, name, path, store, true, null, optional, params, close, reg, log, true);
}
 ...
 */
async function defaultWriter(name, path, optional, params, store, data, writeFormatter, close, log) {
  const mods = await modules(optional), reg = initRegistrant(path, optional, store, null, writeFormatter);
  return writeAndSet(mods, name, path, optional, params, store, data, false, null, reg, log);
}
...
async function readAndSet(mods, name, path, store, forContent, extension, optional, params, close, registrant, log, isRender) {
  ...
  if (forContent) {
    ...
    store[cacheName][path] = { name: path, shortName: name, func: await registrant.modularize(path, true) };
  }
 ...
 * @returns {Function} The modularized function
 */
async function modularize(name, store, clear) {
  // not really much to do here since modules/functions are created/stored in memory
  return store.sources && store.sources[name] && store.sources[name].func;
}
