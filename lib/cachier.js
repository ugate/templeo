...
// TODO : ESM use... export class Cachier {
  ...
  constructor(opts, formatFunc, isFormatOnRead = true, fetchPartials = false, postPartials = false) {
    ...
    ns.at.fetchPartials = fetchPartials;
    ns.at.postPartials = postPartials;
  ...
  async write(name, data, forContent) {
    ...
    else ns.at.dbx[cacheName][id].func = ns.at.options.useCommonJs ? require(id) : await import(id);
  }
...
async function fetcher(ns, url, data) {
  ...
  return new Promise(async (resolve, reject) => {
    ...
    const req = https.request(url, { method: (data && 'POST') || 'GET' }, res => {
      var data = '';
      res.on('data', chunk => {
        data += chunk;
      });
