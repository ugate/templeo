  ...
  async read(name, forContent, extension, params) {
    ...
    if (ns.at.logger.debug) {
      ns.at.logger.debug(`Loading template ${forContent ? 'partial' : 'code'} for name: "${name}"`
        + ` from ${isFetch ? `GET ${rwname}${params ? ` PARAMS: ${params.toString()}` : ''}` : `module @ ${name}`}`);
    }
    if (forContent) {
...
function readWriteName(name, optional, params, forContent, extension, forContext) {
  ...
  var ext;
  var realName = name;//const url = new URL(/^https?:\/?\/?/i.test(name) ? name : `http://example.com/${name}`), realName = url.pathname.replace('/', '');
  if (/\..*$/.test(realName)) {
  ...
  return `${pathBase}${realName}${ext}${params instanceof URLSearchParams ? `?${params.toString()}` : ''}`;
}
