 ...
 * the standard Fetch API in supported Node.js and browser runtimes to capture partial template content. Compiled template
...
class Cachier {
  ...
  async unregister(name) {
    ...
    const contentPath = await ns.this.readWriteName(name, opts, null, ns.at, true);
    const sourcePath = await ns.this.readWriteName(name, opts, null, ns.at, false);
    const contentBase = contentPath && contentPath.split('?')[0];
    const removeMatches = (collection, exact, base) => {
      if (!ns.at[collection]) return;
      for (const path of Object.keys(ns.at[collection])) {
        if (path === name || path === exact || (base && (path === base || path.startsWith(`${base}?`)))) {
          cacheRemove(ns.at, collection, path);
        }
      }
    };
    removeMatches('data', contentPath, contentBase);
    removeMatches('sources', sourcePath, sourcePath && sourcePath.split('?')[0]);
  }
  ...
  async registerPartial(name, contentOrParams, extension) {
    ...
    const params = contentOrParams instanceof URLSearchParams ? contentOrParams : null;
    ...
    const registered = ns.at.data[path];
    cacheTouch(ns.at, 'data', path, registered, opts);
  ...
  async getRegistered(name, params, extension) {
    ...
    const entry = path && cacheLookup(ns.at, 'data', path, opts);
    if (!entry) return null;
    const copy = { ...entry };
    if (entry.params instanceof URLSearchParams) copy.params = new URLSearchParams(entry.params);
    if (entry.content && typeof entry.content === 'object') copy.content = JSON.parse(JSON.stringify(entry.content));
    return copy;
  }
  ...
  async compile(name, template, params, extension) {
    ...
    const compileKey = cacheOperationKey('compile', name, params, `${extension || ''}:${template === true ? 'read' : typeof template === 'string' ? template : typeof template}`);
    return withPending(ns.at, 'compiles', compileKey, async () => {
      cacheMetric(ns.at, 'compiles');
      let content = template, cnm, func;
      if (template === true) {
        let rd;
        cnm = await ns.this.readWriteName(name, opts, params, ns.at, true, extension);
        const cachedContent = opts.cacheRawTemplates ? cacheLookup(ns.at, 'data', cnm, opts) : null;
        if (!cachedContent || !Object.prototype.hasOwnProperty.call(cachedContent, 'content')) {
          rd = await ns.this.read(name, true, extension, params);
          content = rd && (Object.prototype.hasOwnProperty.call(rd, 'content') ? rd.content : rd.data && rd.data[cnm] ? rd.data[cnm].content : null);
          if (typeof content !== 'string') {
        ...
        } else content = cachedContent.content;
        if (content) {
          const snm = await ns.this.readWriteName(name, opts, params, ns.at, false);
          const cachedSource = opts.cacheRawTemplates ? cacheLookup(ns.at, 'sources', snm, opts) : null;
          if (!cachedSource || !Object.prototype.hasOwnProperty.call(cachedSource, 'func')) {
            try {
   ...
   * When `options.partialsURL` is set to an HTTP/S URL and the read is for partial content, a `GET` call to the standard Fetch API is made. The partial `name` will be appended to
   ...
   * When `options.partialsURL` is set to an HTTP/S URL and the write is for partial content, a `POST` call to the standard Fetch API is made. The partial `name` will be appended to
   * `options.partialsURL` (e.g. `https://example.com/some/id.html` where `some/id.html` is the the partial's
   * {@link Cachier.readWriteName} and `options.partialsURL` is set to `https://example.com`). When `options.partialsURL` is
   ...
   */
  get stats() {
    return cacheSnapshot(internal(this).at);
  }

  /**
   * Resets cumulative cache counters while retaining current entry and byte totals.
   * @returns {Object} The reset cache statistics.
   */
  resetStats() {
    return cacheResetStats(internal(this).at);
  }
  ...
  /**
   * Clears the cache.
   * @param {Boolean} [all=false] Reserved for persistence implementations that can clear unassociated cache instances.
   */
  ...
  static async waiter(proms, errMsg = 'One or more pomises failed', capture = true) {
    ...
    for (let prom of proms) {
      ...
      try {
        ...
        const nerr = new Error(err.constructor.name !== 'Error' ? `${err.constructor.name || 'PromiseError'}:` : '');
        nerr.code = err.code;
...
async function readAndSet(mods, name, path, store, forContent, extension, optional, params, close, registrant, log, isCompile) {
  ...
  return withPending(store, 'reads', pendingKey, async () => {
    ...
    let result;
    if (forContent) {
      const cached = !isFetch ? cacheLookup(store, cacheName, path, optional) : null;
      let content = isFetch ? await fetcher(mods, path, optional, params, store, null, registrant.readFormatter, close, log) : cached && cached.content;
      ...
      result = store[cacheName] && store[cacheName][path];
      if (!result) result = { name: path, shortName: name, content, extension };
      cacheTouch(store, cacheName, path, result, optional);
    } else {
      const cached = cacheLookup(store, cacheName, path, optional);
      result = {
        name: path,
        shortName: name,
        func: cached && cached.func
      };
      store[cacheName][path] = result;
      cacheTouch(store, cacheName, path, result, optional);
    }
    return result;
  });
...
async function writeAndSet(mods, name, path, optional, params, store, data, forContent, extension, registrant, log, isCompile) {
  ...
  return withPending(store, 'writes', pendingKey, async () => {
    ...
    let writable = isContentJSON ? data : dataType === 'function' ? data.toString() : dataType === 'string' ? data : null;
    if (log && log.info) {
    ...
    if (writable && isFetch) {
      ...
      writable = registrant.writeFormatter(writable, wopts);
    }
    if (forContent) {
      ...
      if (store[cacheName] && store[cacheName][path]) cacheTouch(store, cacheName, path, store[cacheName][path], optional);
    } else {
      store[cacheName] = store[cacheName] || Object.create(null);
      store[cacheName][path] = {
        name: path,
        shortName: name,
        func: typeof (writable || data) === 'function' ? writable || data : (new Function(`return ${writable || data}`))()
      };
      cacheTouch(store, cacheName, path, store[cacheName][path], optional);
...
/**
 * GETs or POSTs data using the standard Fetch API available in supported Node.js and browser runtimes.
 * @private
 * @ignore
...
async function fetcher(mods, url, optional, params, store, data, formatter, close, log) {
  ...
  if (typeof fetch !== 'function' || typeof Request !== 'function') {
    throw new Error('Templeo requires the standard Fetch API (global fetch and Request) for HTTP/S template operations');
  }
  ...
  if (ropts.method === 'GET' && params) {
    const existing = new Map(), consumed = new Map();
    for (const [key, value] of url.searchParams) {
      const pair = `${key}\u0000${value}`;
      existing.set(pair, (existing.get(pair) || 0) + 1);
    }
    for (const [key, value] of params) {
      const pair = `${key}\u0000${value}`, used = consumed.get(pair) || 0, available = existing.get(pair) || 0;
      if (used < available) consumed.set(pair, used + 1);
  ...
  if (log && (log.info || log.debug)) {
    (log.debug || log.info)(`CACHE: 📡 fetch ${ropts.method} "${url.toString()}"`
  ...
  let rtn = await res.text();
  if (log && (log.info || log.debug)) {
    (log.debug || log.info)(`CACHE: 📡 fetch ${ropts.method} "${url.toString()}" returned ${log.debug ? rtn : res.ok ? 'OK' : 'Not OK'}`);
  }
...
async function readWriteName(nmrs, name, optional, params, store, forContent, extension, forContext) {
  ...
  let realName = changed ? url.toString() : name;
  if (changed && !isAbs) realName = realName.replace(`${url.origin}/`, '');
