...
// TODO : ESM use... export class Cachier {
   ...
   * @returns {Function} [readers[].start] An `async function` that can perform initialization tasks for a reader
   * __before rendering has started__. Arguments passed are `storage`, `optional` and `log` as described for `readers[].read`. Returning
   * `true` will stop any further rendering from processing resulting in the rendering function returning a _blank_ string.
   * @returns {Function} [readers[].finish] An `async function` that can perform cleanup tasks for a reader
   * __after rendering has completed__. Arguments passed are `storage`, `optional` and `log` as described for `readers[].read`.
  ...
  async readWriteName(partialName, optional, params, storage, forContent, extension, forContext) {
    ...
    return nmrs.namer(nmrs, partialName, optional, params, storage, forContent, extension, forContext);
  }
   ...
   * @returns {Object} `namers.namerOnReadError` The naming function to use when a {@link Cachier.readers} function throws an error. The next
   * reader called in the {@link Cachier.readers} list will use the name generated by this reader.
   */
  get readWriteNames() {
...
function isAbsName(name, opts) {
  const hasBase = opts.bypassUrlRegExp && name.match(opts.bypassUrlRegExp);
  return !!(hasBase || opts.partialsURL);
}
