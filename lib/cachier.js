...
// TODO : ESM use... export class Cachier {
   ...
   * @param {Object} [params] The JSON parameters that will be in-scope within the partial as `options.includesParametersName`
  ...
  registerPartial(name, content, extension, params) {
    ...
    if (params && typeof params === 'object') {
      ns.at.dbx[inm][name].params = JSON.parse(JSON.stringify(params));
    }
  ...
  async registerPartials(partials, read) {
    ...
    for (let prtl of partials) {
      ...
      ns.this.registerPartial(prtl.name, content, prtl.extension, prtl.params);
      rtn.partials.push({ name: prtl.name, content, extension: prtl.extension, params: prtl.params });
    }
    return rtn;
  }
   ...
   */
  async compile(name, template, extension, DirClass) {
    const ns = internal(this), inm = ns.at.names.incls, rnm = ns.at.names.rndrs;
    ...
    if (!func) { // no rendering function requires a fresh compile
      func = Sandbox.compile(name, content, ns.at.dbx[inm], ns.at.dbx[rnm], ns.at.options, ns.this.readWriteName, ns.this.reader, DirClass);
 ...
 * @param {(Object | String)} [params] The URL parameters to use (JSON or URL encoded)
 * @param {Object} [headers] The headers to use
 * @param {String} [method=GET] The method to use
 * @returns {String} The result
 */
async function fetcher(url, optional, params, headers = { 'Content-Type': 'text/html; charset=utf-8' }, method = 'GET') {
  method = (method && method.toLocaleUpperCase()) || 'GET';
  const isOptionFunc = typeof optional === 'function';
  const paramsType = typeof params;
  url = new URL(url);
  if (method === 'GET' && params && paramsType === 'object') {
  ...
  if (typeof fetch !== 'undefined') {
    ...
    if (method === 'POST') {
      fetchOpts.body = encodeURIComponent(paramsType === 'object' ? JSON.stringify(params) : params);
    }
    const res = await fetch(new Request(url.toString()), fetchOpts);
    if (!res.ok) {
