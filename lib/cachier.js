...
// TODO : ESM use... export class Cachier {
   ...
   */
  constructor(opts, formatFunc, isFormatOnRead = true, log = {}) {
  ...
  async registerPartial(name, contentOrParams, extension) {
    ...
    const rwname = await ns.this.readWriteName(name, opts, params, ns.at, true, extension);
  ...
  async compile(name, template, params, extension) {
    ...
    if (!hasTmpl) { // partial template includes
      ...
      if (!opts.cacheRawTemplates || !ns.at.dbx.partials[cnm] || !ns.at.dbx.partials[cnm].hasOwnProperty('content')) {
      ...
      } else content = ns.at.dbx.partials[cnm].content;
      // read/load rendering function?
      const snm = await ns.this.readWriteName(name, opts, params, ns.at, false);
      if (rd || !opts.cacheRawTemplates || !ns.at.dbx.sources[snm] || !ns.at.dbx.sources[snm].hasOwnProperty('func')) {
        const rd = await ns.this.read(name, false, extension, params);
        func = rd && rd.func;
      } else func = ns.at.dbx.sources[snm].func;
    }
    if (!func) { // no rendering function requires a fresh compile
      func = Sandbox.compile(name, content, ns.at.dbx.partials, ns.at.dbx.sources, opts, ns.this.readWriteNames,
          ns.this.readers, ns.at.dbx.helpers, ns.this.metadata, log);
      ...
      if (func) {
        ...
        if (ns.this.isWritable) await ns.this.write(name, func, false, null, params);
      }
    }
    return func;
  }
  ...
  async read(name, forContent, extension, params) {
    const ns = internal(this), opts = ns.this.options, log = ns.this.log, cacheName = forContent ? 'partials' : 'sources';
    const isFetch = forContent && isAbsName(name, opts);
    const rwname = await ns.this.readWriteName(name, opts, params, ns.at, forContent, extension);
    if (log.info) {
      log.info(`Reading template ${forContent ? 'partial' : 'code'} `
        + `from ${isFetch ? `GET "${rwname}${params ? `?${params.toString()}"` : ''}` : `module @ ${name}`}`);
    }
    if (forContent) {
      const content = isFetch ? await fetcher(rwname, opts, params) : ns.at.dbx[cacheName][rwname] && ns.at.dbx[cacheName][rwname].content;
      await ns.this.registerPartial(name, typeof content !== 'string' ? params : content, extension);
    } else {
      ns.at.dbx[cacheName][rwname] = { name: rwname, shortName: name, func: await ns.this.modularize(name, true) };
    }
    return ns.at.dbx[cacheName][rwname];
  }
  ...
  async write(name, data, forContent, extension, params) {
    ...
    var writable = dataType === 'function' && ((isFetch && rwname) || forContent) ? Sandbox.serialzeFunction(data) : null;
    if (writable && !ns.at.isFormatOnRead && ns.at.formatter) {
      writable = ns.at.formatter(writable, opts.formatOptions);
    }

    if (log.info) {
      log.info(`Writting template ${forContent ? 'partial' : 'code'} ${isFetch ? `POST "${rwname}"` : 'for "${rwname}" to memory'}`);
    }
    if (isFetch) await fetcher(rwname, opts, writable, undefined, 'POST');
    if (forContent) {
      await ns.this.registerPartial(name, typeof writable !== 'string' ? params : writable, extension);
    } else {
      ns.at.dbx[cacheName][rwname] = {
        name: rwname,
        shortName: name,
   ...
   * render-time reads
   * @returns {Function} [readers[].read] The reader is an `async function` responsible for reading partial template content/modules/etc
   ...
   * @returns {Function} [readers[].start] An `async function(storage:Object, optional:(TemplateOpts | Function(name:String):*), log:Object)`
   * that can perform initialization tasks for a reader __before rendering has started__
   * @returns {Function} [readers[].finish] An `async function(storage:Object, optional:(TemplateOpts | Function(name:String):*), log:Object)`
   * that can perform cleanup tasks for a reader __after rendering has completed__
   * @returns {Function[]} [readers[].scopes] Zero or more functions that will be in scope when the read function is called.
   ...
   */
  async readWriteName(partialName, optional, params, storage, forContent, extension, forContext) {
    let namers = this.readWriteNames, named;
    if (!Array.isArray(namers)) namers = [namers];
    for (let namer of namers) {
      named = await namer(partialName, optional, params, storage, forContent, extension, forContext, named);
    }
    return named;
  }
   ...
   * @returns {(Function | Function[])} One or more async functions responsible for formatting template names into a full path name
 ...
 * @param {Object} [storage] The object where the `storage.partials` are stored
 ...
 */
async function fetcher(url, optional, params, storage, close, log) {
 ...
 * @param {Object} storage The storage object that can contain metadata for naming operations
 ...
 */
async function readWriteName(name, optional, params, storage, forContent, extension, forContext) {
  const isOptionFunc = typeof optional === 'function';
  ...
  var ext, pathBase = (!isBypass && (isOptionFunc ? optional(pathOptName) : optional[pathOptName])) || '';
  ...
  return `${pathBase}${realName}${params instanceof URLSearchParams ? `?${params.toString()}` : ''}`;
}
