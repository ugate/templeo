...
// TODO : ESM use... export class Cachier {
   ...
   */
  constructor(opts, formatFunc, isFormatOnRead = true, servePartials = null) {
    if (!(opts instanceof EngineOpts)) throw new Error(`Options must be of type ${EngineOpts.constructor.name} not ${opts}`);
  ...
  async generateCode(name, code, write = true, store = true) {
    ...
    put.name = name;
    put.id = id;
    rtn.func = Cachier.deserialzeFunction(put.code, name);
  ...
  async read(name, forContent) {
    const ns = internal(this);
    return ns.this.load(name, forContent, !ns.at.options.isCached);
  }
  ...
  async write(name, data, forContent) {
    ...
    const isDataFunc = typeof data === 'function', writable = isDataFunc ? Cachier.serialzeFunction(data) : data;
    ...
    else ns.at.dbx[cacheName][id].func = isDataFunc ? data : Cachier.deserialzeFunction(data, name);
  }
  ...
  async load(name, forContent, refresh) {
    ...
    if (url) {
      ns.at.dbx[cacheName][id] = { name, id, content: await fetcher(ns, `${url}/${id}`, null, rejUnauth) };
    } else if (!forContent) {
      if (refresh) clearModule(ns, id);
      ns.at.dbx[cacheName][id] = { name, id, func: ns.at.options.useCommonJs ? require(id) : /* TODO : ESM use... await import(id)*/null };
    }
    return ns.at.dbx[cacheName][id];
  }
...
async function fetcher(ns, url, data, rejectUnauthorized = true) {
  ...
  if (ns.at.options.logger.info) {
    ns.at.options.logger.info(`Template ${method} request: ${url}`);
  }
