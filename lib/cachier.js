...
// TODO : ESM use... export class Cachier {
  ...
  async read(name, forContent, extension, params) {
    ...
    if (log.info) {
      log.info(`Reading template ${forContent ? 'partial' : 'code'} `
        + `from ${isFetch ? `GET "${path}${params ? `?${params.toString()}"` : ''}` : `module @ ${name}`}`);
    }
    if (forContent) {
      const formatter = ns.this.readFormatter;
      let content = isFetch ? await fetcher(await modules(opts), path, opts, params, ns.at, null, false, log) : 
        ns.at.dbx[cacheName][path] && ns.at.dbx[cacheName][path].content;
      ...
      ns.at.dbx[cacheName][path] = { name: path, shortName: name, func: await ns.this.modularize(name, true) };
    }
  ...
  async write(name, data, forContent, extension, params) {
    ...
    var writable = dataType === 'function' && ((isFetch && path) || forContent) ? Sandbox.serialzeFunction(data) : null;
    if (writable && !forContent && formatter) {
      writable = formatter(writable, opts.writeFormatOptions);
    }

    if (log.info) {
      log.info(`Writting template ${forContent ? 'partial' : 'code'} ${isFetch ? `POST "${path}"` : `for "${path}" to memory`}`);
    }
    if (isFetch) await fetcher(await modules(opts), path, opts, params, ns.at, writable, null, true, log);
    if (forContent) {
      await ns.this.registerPartial(name, typeof writable !== 'string' ? params : writable, extension);
    } else {
      ns.at.dbx[cacheName][path] = {
        name: path,
        shortName: name,
