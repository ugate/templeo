...
// TODO : ESM use... export class Cachier {
  ...
  async registerPartials(partials, read) {
    ...
    for (let prtl of partials) {
      if (!prtl.hasOwnProperty('name')) {
        ...
        if (read) {
          content = await ns.this.read(prtl.name, true);
        ...
        if (!content) {
          ...
          try {
            new URL(prtl.name, ns.at.options.pathBase || undefined); // validate the name/pathBase URL for consistency with runtime errors
          } catch (err) {
            err.message = `${err.message} CAUSE: ${errMsg}`;
            throw err;
          }
          throw new Error(errMsg);
        }
      } else content = prtl.content;
      ns.this.registerPartial(prtl.name, content);
      rtn.partials.push({ name: prtl.name, content });
    }
    return rtn;
  }
   ...
   * When `options.pathBase` is set to a URL base and the read is for partial content, A `GET` call to `window.fetch` is made when running
  ...
  async read(name, forContent) {
    ...
    const base = forContent && ns.at.options.pathBase;
    const url = base ? ns.this.readWriteName(name, ns.at.options, null, forContent) : null;
    ...
    if (forContent) ns.at.dbx[cacheName][name].content = url ? await fetcher(url, ns.at.options) : ns.at.dbx[cacheName][name].content;
   ...
   * When `options.pathBase` is set to a URL base and the read is for partial content, A `POST` call to `window.fetch` is made when running
   * within browsers or to the `https` module when running on the server. The partial `name` will be appended to `options.pathBase` (e.g.
   * `https://example.com/some/id.html` where `some/id.html` is the the partial's {@link Cachier.readWriteName} and `options.pathBase` is set
  ...
  async write(name, data, forContent) {
    ...
    if (url) await fetcher(url, ns.at.options, null, writable);
    ns.at.dbx[cacheName][name] = { name };
 ...
 */
async function fetcher(url, compileOptions, renderOptions, data) {
  ...
  return new Promise(function httpsFetcher(resolve, reject) {
    const useCommonJs = renderOptions.hasOwnProperty('useCommonJs') ? renderOptions.useCommonJs : compileOptions.useCommonJs;
    const rejectUnauthorized = (renderOptions.hasOwnProperty('rejectUnauthorized') ? 
      renderOptions.rejectUnauthorized : compileOptions.rejectUnauthorized) !== false;
    ...
    const req = https.request(url, { method, rejectUnauthorized }, res => {
      ...
      if (res.statusCode < 200 || res.statusCode > 299) {
        return reject(new Error(`${res.statusCode}: ${res.statusMessage || ''} CAUSE: template content ${method} failed for: ${url}"`));
      }
      res.on('data', chunk => {
        data += chunk;
      });
 ...
 */
function readWriteName(name, compileOptions, renderOptions, forContent) {
  renderOptions = renderOptions || {};
  const pathBase = (renderOptions.hasOwnProperty('pathBase') ? renderOptions.pathBase : compileOptions.pathBase) || '';
  const base = pathBase ? `${pathBase}${pathBase.endsWith('/') ? '' : '/'}` : '';
  var ext;
  if (/\..*$/.test(name)) {
    ext = ''; // already has extension
  } else if (forContent) {
    ext = (renderOptions.hasOwnProperty('defaultExtension') ? renderOptions.defaultExtension : compileOptions.defaultExtension) || '';
  } else {
    ext = renderOptions.hasOwnProperty('useCommonJs') ? renderOptions.useCommonJs : compileOptions.useCommonJs ? 'js' : 'mjs';
  }
  if (ext) ext = `.${ext}`;
