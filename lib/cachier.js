...
// TODO : ESM use... export class Cachier {
   ...
   */
  constructor(opts, formatFunc, isFormatOnRead = true, fetchPartials = false, postPartials = false) {
    const ns = Cachier.internal(this);
    ns.at.options = opts instanceof EngineOpts ? opts : new EngineOpts(opts);
    ns.at.fetchPartials = fetchPartials && 'GET';
    ns.at.postPartials = postPartials && 'POST';
  ...
  async scan(registerPartial, unregisterPartial) {
    ...
    if (registerPartial && Array.isArray(ns.at.options.partials) && ns.at.options.partials.length) {
      ...
      return rtn;
    } else if (ns.at.options.logger.info) ns.at.options.logger.info(`No options.partials to process`);
  }
  ...
  async read(name, forContent) {
    return this.load(name, forContent);
  }
  ...
  async write(name, data, forContent) {
    ...
    const ns = Cachier.internal(this), id = ns.this.identity(name), cacheName = forContent ? ns.at.names.partials : ns.at.names.code;
    if (ns.at.options.logger.debug) {
      ns.at.options.logger.debug(`Writting template ${forContent ? 'partial' : 'code'} for "${name}" (name)/"${id}" (ID)`
        + ` to ${forContent && ns.at.postPartials ? `${ns.at.postPartials} ${id}` : 'memory'}`);
    }
    if (forContent && ns.at.postPartials) await fetcher(ns, id, data);
    ns.at.dbx[cacheName][id] = { name, id };
    if (forContent) ns.at.dbx[cacheName][id].content = data;
    else ns.at.dbx[cacheName][id].func = Cachier.deserialzeFunction(data);
  }
  ...
  async load(name, forContent, refresh) {
    const ns = Cachier.internal(this), id = ns.this.identity(name), cacheName = forContent ? ns.at.names.partials : ns.at.names.code;
    if ((!refresh && ns.at.dbx[cacheName][id]) || refresh === Cachier.LOAD_REFRESH_NEVER) {
      if (ns.at.options.logger.debug) {
        ns.at.options.logger.debug(`Reading template ${forContent ? 'partial' : 'code'} for "${name}" (name)/"${id}" (ID) from memory'}`);
      }
      return ns.at.dbx[cacheName][id];
    }
    if (ns.at.options.logger.debug) {
      ns.at.options.logger.debug(`Loading template ${forContent ? 'partial' : 'code'} for "${name}" (name)/"${id}" (ID)`
        + ` from ${forContent && ns.at.fetchPartials ? `${ns.at.fetchPartials} ${id}` :
        `${ns.at.options.useCommonJs ? 'CommonJS' : 'ECMAScript'} Module @ ${id}`}`);
    }
    if (forContent && ns.at.fetchPartials) {
      ns.at.dbx[cacheName][id] = { name, id, content: await fetcher(ns, id) };
    } else if (!forContent) {
   ...
   */
  static deserialzeFunction(str) {
    return str && typeof str === 'string' ? new Function(`return ${str.toString()}`)() : null;
  }
 ...
 */
async function fetcher(ns, url, data) {
  ...
  return new Promise(async (resolve, reject) => {
    const https = ns.at.options.useCommonJs ? require('https') : await import('https');
    const req = https.request(url, { method: (hasData && 'POST') || 'GET' }, res => {
      var data = '';
      res.on('data', chunk => {
        data += chunk;
      });
