...
// TODO : ESM use... export class Cachier {
  ...
  async read(name, forContent, extension, params) {
    const ns = internal(this), opts = ns.this.options, log = ns.this.log, formatter = ns.this.readFormatter, cacheName = forContent ? 'partials' : 'sources';
    ...
    if (forContent) {
      let content = isFetch ? await fetcher(rwname, opts, params) : ns.at.dbx[cacheName][rwname] && ns.at.dbx[cacheName][rwname].content;
      const ctype = typeof content;
      if (ctype === 'string' && forContent && formatter) {
        content = formatter(content, opts.formatOptions);
      }
  ...
  async write(name, data, forContent, extension, params) {
    ...
    data = data || '';
    const ns = internal(this), opts = ns.this.options, log = ns.this.log, formatter = ns.this.writeFormatter, cacheName = forContent ? 'partials' : 'sources';
    const isFetch = forContent && isAbsName(name, opts);
    const rwname = await ns.this.readWriteName(name, opts, params, ns.at, forContent, extension);
    ...
    if (writable && !forContent && formatter) {
      writable = formatter(writable, opts.formatOptions);
    }
   ...
   * that will be formatted and the second being `options.formatOptions`
   */
  get readFormatter() {
    const ns = internal(this);
   ...
   * takes 1 or 2 arguments with the first being the content that will be formatted and the second being `options.formatOptions`
 ...
 */
async function fetcher(url, optional, params, store, formatter, close, log) {
  if (!url) return;
