...
// TODO : ESM use... export class Cachier {
  ...
  registerPartial(name, contentOrParams, extension) {
    const ns = internal(this), ext = (extension && String(extension)) || ns.at.options.defaultExtension || '';
    const params = contentOrParams instanceof URLSearchParams ? params : null;
    const rwname = ns.this.readWriteName(name, ns.at.options, params, true, extension);
  ...
  async compile(name, template, extension) {
    ...
    if (!hasTmpl) { // partial template includes
      ...
      if (!ns.at.options.cacheRawTemplates || !ns.at.dbx.partials[name] || !ns.at.dbx.partials[name].hasOwnProperty('content')) {
      ...
      if (rd || !ns.at.options.cacheRawTemplates || !ns.at.dbx.sources[name] || !ns.at.dbx.sources[name].hasOwnProperty('func')) {
    ...
    if (!func) { // no rendering function requires a fresh compile
      func = Sandbox.compile(name, content, ns.at.dbx.partials, ns.at.dbx.sources, ns.at.options, ns.this.readWriteName,
  ...
  async read(name, forContent, extension, params) {
    const ns = internal(this), cacheName = forContent ? 'partials' : 'sources';
    const url = forContent && isAbsName(name, ns.at.options) ? ns.this.readWriteName(name, ns.at.options, params, forContent, extension) : null;
  ...
  /**
   * @returns {(Function | Function[] | Object | Object[])} One or more 
   ...
   * responsible for reading partial template content/modules/etc. when a template `include` cannot be found within the compilation
   * cache (__render-time__ loading). The functions should not reference any external scope other than the global object space.
   * When returning multiple functions, they should be in the order in which they will be executed. Each entry can also be an
   * `{ read:Function, finish:Function, readAllOnInitWhenEmpty:Boolean }` where read is the aforementioned reader function, __finish__
   * is an `async function(storage:Object)` that can perform cleanup tasks for each reader after rendering has completed and
   * __readAllOnInitWhenEmpty__ is a flag that indicates `read` will be invoked without a _name_ parameter when no partials exist
   * during initialization. The __storage__ object can contain metadata for read operations and `storage.partials` should contain each
   * of the read paratial template metadata. Read functions can return the partial template content and/or it can be set on the
   * `storage.partials`.
   */
  get readers() {
    return fetcher;
