...
// TODO : ESM use... export class Cachier {
  ...
  /**
   * Determines if a template, partial or context __exists in-memory__.
   * @param {String} name The __raw__ template name (i.e. not from {@link Cachier.readWriteName})
   ...
   * @returns {Boolean} `true` the template, partial or context is registered
   */
  async isRegistered(name, params, extension) {
    ...
    return path && ns.at.data[path] && !!ns.at.data[path].content;
  }
   ...
   * @param {String} [template] The template to be compiled. Omit to load the template from cache.
  ...
  async compile(name, template, params, extension) {
    ...
    if (!func) { // no rendering function requires a fresh compile
      func = Sandbox.compile(name, content, ns.at.data, ns.at.sources, opts, ns.this.readWriteNames,
