...
// TODO : ESM use... export class Cachier {
  ...
  async compile(name, template, params, extension) {
    ...
    if (!hasTmpl) { // partial template includes
      ...
      cnm = ns.this.readWriteName(name, opts, params, true, extension);
      ...
      const snm = ns.this.readWriteName(name, opts, params, false);
    ...
    if (!func) { // no rendering function requires a fresh compile
      ...
      if (func) {
        if (ns.at.logger.info) ns.at.logger.info(`Compiled template named "${func.name}"${cnm ? ` @ "${cnm}"` : ''}`);
  ...
  async read(name, forContent, extension, params) {
    ...
    const rwname = ns.this.readWriteName(name, opts, params, forContent, extension);
    if (ns.at.logger.info) {
      ns.at.logger.info(`Reading template ${forContent ? 'partial' : 'code'} `
  ...
  async write(name, data, forContent, extension, params) {
    ...
    if (ns.at.logger.info) {
      ns.at.logger.info(`Writting template ${forContent ? 'partial' : 'code'} ${isFetch ? `POST "${rwname}"` : 'for "${rwname}" to memory'}`);
    }
  ...
  /**
   * @returns {Function} [namer] The function responsible for formatting template names into a full path name consumable by `read`/`write`
