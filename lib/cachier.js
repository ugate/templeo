...
// TODO : ESM use... export class Cachier {
  ...
  registerPartial(name, contentOrParams, extension) {
    const ns = internal(this);
    ns.at.dbx.partials[name] = { name };
    ns.at.dbx.partials[name].extension = (extension && String(extension)) || ns.at.options.defaultExtension || '';
    if (contentOrParams instanceof URLSearchParams) {
      ns.at.dbx.partials[name].params = contentOrParams;
    } else if (typeof contentOrParams === 'string') {
      ns.at.dbx.partials[name].content = contentOrParams;
    }
    return ns.at.dbx.partials[name].content;
  }
  ...
  async compile(name, template, extension) {
    ...
    if (!func) { // no rendering function requires a fresh compile
      func = Sandbox.compile(name, content, ns.at.dbx.partials, ns.at.dbx.sources, ns.at.options, ns.this.readWriteName,
          ns.this.readers, ns.at.dbx.helpers);
   ...
   * `{ read:Function, finish:Function, readAllOnInit:Boolean }` where read is the aforementioned reader function, __finish__ is an
   * `async function(storage:Object)` that can perform cleanup tasks for each reader after rendering has completed and __readAllOnInit__
   * is a flag that indicates `read` will be invoked without a _name_ parameter during initialization. The __storage__
   * object can contain metadata for read operations and `storage.partials` should contain each of the read paratial template metadata.
