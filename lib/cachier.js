...
// TODO : ESM use... export class Cachier {
  ...
  async compile(name, template, params, extension) {
    ...
    if (!func) { // no rendering function requires a fresh compile
      func = Sandbox.compile(name, content, ns.at.dbx.partials, ns.at.dbx.sources, opts, ns.this.readWriteNames,
          ns.this.operations, ns.at.dbx.helpers, ns.this.metadata, log);
  ...
  async read(name, forContent, extension, params) {
    ...
    if (forContent) {
      const formatter = ns.this.readFormatter;
      let content = isFetch ? await fetcher(await modules(opts), rwname, opts, params, ns.at, null, false, log) : 
        ns.at.dbx[cacheName][rwname] && ns.at.dbx[cacheName][rwname].content;
      const ctype = typeof content;
      if (ctype === 'string' && formatter) {
        content = formatter(content, opts.readFormatOptions);
      }
      await ns.this.registerPartial(name, ctype !== 'string' ? params : content, extension);
    } else {
  ...
  async write(name, data, forContent, extension, params) {
    const ns = internal(this), opts = ns.this.options, log = ns.this.log;
    if (!opts.cacheRawTemplates) {
      if (log && log.info) log.info(`Skipping write due to "options.cacheRawTemplates" is turned off for template @ "${name}"`)
      return;
    }
    const formatter = ns.this.writeFormatter, cacheName = forContent ? 'partials' : 'sources';
    data = data || '';
    const isFetch = forContent && isAbsName(name, opts);
    const rwname = await ns.this.readWriteName(name, opts, params, ns.at, forContent, extension);
    ...
    if (writable && !forContent && formatter) {
      writable = formatter(writable, opts.writeFormatOptions);
    }

    if (log.info) {
      log.info(`Writting template ${forContent ? 'partial' : 'code'} ${isFetch ? `POST "${rwname}"` : `for "${rwname}" to memory`}`);
    }
    if (isFetch) await fetcher(await modules(opts), rwname, opts, params, ns.at, writable, null, true, log);
    if (forContent) {
   ...
   */
  async modularize(name, clear) {
    const ns = internal(this), store = ns.at;
    // not really much to do here since modules/functions are created/stored in memory
    return store.dbx && store.dbx.sources && store.dbx.sources[name] && store.dbx.sources[name].func;
  }
 ...
 */
async function defaultReader(name, optional, params, store, readFormatter, close, log) {
  if (!name) return;
  return fetcher(await modules(optional), name, optional, params, store, null, readFormatter, close, log);
}
 ...
 */
async function defaultWriter(name, optional, params, store, data, writeFormatter, close, log) {
  return writeAndSet(await modules(optional), name, optional, params, store, data, false, extension, { writeFormatter }, log);
}
 ...
 * @param {*} log 
 */
async function writeAndSet(mods, name, optional, params, store, data, forContent, extension, registrant, log) {
  ...
  const dataType = typeof data;
  let writable = dataType === 'function' && ((isFetch && name) || forContent) ? data.toString() : null;
  if (writable && !forContent && typeof registrant.writeFormatter === 'function') {
    writable = registrant.writeFormatter(writable, opts.writeFormatOptions);
  }

  if (log.info) {
    log.info(`Writting template ${forContent ? 'partial' : 'code'} ${isFetch ? `POST "${name}"` : `for "${name}" to memory`}`);
  }
  if (isFetch) await fetcher(mods, name, optional, params, store, writable, registrant.writeFormatter, true, log);
  if (forContent) {
    await registrant.registerPartial(name, typeof writable !== 'string' ? params : writable, extension);
  } else {
    store[cacheName] = store[cacheName] || {};
    store[cacheName][name] = {
      name,
      func: dataType === 'function' ? writable || data : (new Function(`return ${writable || data}`))()
    };
    return store[cacheName][name].func;
  }
}
