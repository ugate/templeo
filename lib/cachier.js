...
// TODO : ESM use... export class Cachier {
  ...
  async compile(name, template, extension, DirClass) {
    ...
    if (!func) { // no rendering function requires a fresh compile
      func = Sandbox.compile(name, content, ns.at.dbx[inm], ns.at.dbx[rnm], ns.at.options, ns.this.readWriteName, fetcher, DirClass);
...
function readWriteName(name, optional, forContent, extension, forContext) {
  ...
  if (/\..*$/.test(name)) {
    ...
    ext = typeof extension === 'string' ? extension : (forContext ? 'json' : isOptionFunc ? optional('defaultExtension') : optional.defaultExtension) || '';
  } else {
