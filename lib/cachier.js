...
// TODO : ESM use... export class Cachier {
  ...
  async registerPartial(name, contentOrParams, extension) {
    ...
    if (params) {
      ...
      ns.at.partials[path].content = JSON.parse(JSON.stringify(contentOrParams));
    }
  ...
  async registerPartials(partials, read, write) {
    ...
    for (let prtl of partials) {
      pidx++;
      ext = (prtl.extension && String(prtl.extension)) || opts.defaultExtension || '';
      ...
      if (!prtl.hasOwnProperty('content')) {
        if (read) {
          content = await ns.this.read(prtl.name, true, ext, params);
        ...
        if (write) await ns.this.write(prtl.name, content, true, ext);
      }
      await ns.this.registerPartial(prtl.name, typeof content !== 'undefined' ? content : prtl.params, ext);
      rtn.partials.push({ name: prtl.name, content, extension: ext, params });
    }
    return rtn;
  }
  ...
  async compile(name, template, params, extension) {
    ...
    if (!hasTmpl) { // partial template includes
      ...
      if (!opts.cacheRawTemplates || !ns.at.partials[cnm] || !ns.at.partials[cnm].hasOwnProperty('content')) {
        ...
        content = rd && (rd.hasOwnProperty('content') ? rd.content : rd.partials && rd.partials[cnm] ? rd.partials[cnm].content : null);
        if (typeof content !== 'string') throw new Error(`Unable to find/read primary template @ "${name}" (full name: "${cnm}")`);
   ...
   * @returns {Function} [operations[].write] The write function that will be used for writting newly discovered templates and/or sources.
   * Accepts the same arguments as `operations[].write`.  and _scoped_ functions will be available.
   ...
   * `true` will stop any further rendering from processing resulting in the rendering function returning a _blank_ string. All functions
   * defined within `operations[].scopes` will be available by name.
...
async function defaultRenderReader(name, path, ext, forContent, optional, params, store, readFormatter, close, log) {
  ...
  return readAndSet(mods, name, path, store, forContent, ext, optional, params, close, reg, log, true);
}
...
function initRegistrant(path, optional, store, readFormatter, writeFormatter) {
  const reg = {
    ...
    unregister: name => {
      return Promise.resolve([delete store.partials[path], delete store.sources[path]]);
    }
  };
