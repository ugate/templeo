...
// TODO : ESM use... export class Cachier {
  ...
  unregister(name) {
    ...
    if (ns.at.dbx[pnm][name]) delete ns.at.dbx[pnm][name];
    if (ns.at.dbx[snm][name]) delete ns.at.dbx[snm][name];
  }
  ...
  registerPartial(name, contentOrParams, extension) {
    const ns = internal(this), pnm = ns.at.options.partialStoreName;
    ns.at.dbx[pnm][name] = { name };
    ns.at.dbx[pnm][name].extension = (extension && String(extension)) || ns.at.options.defaultExtension || '';
    if (contentOrParams instanceof URLSearchParams) {
      ns.at.dbx[pnm][name].params = contentOrParams;
    } else if (typeof contentOrParams === 'string') {
      ns.at.dbx[pnm][name].content = contentOrParams;
    }
    return ns.at.dbx[pnm][name].content;
  }
  ...
  async registerPartials(partials, read) {
    ...
    for (let prtl of partials) {
      if (!prtl.hasOwnProperty('name')) {
        throw new Error(`Template partial "name" missing at index ${rtn[pnm].length} for ${JSON.stringify(prtl)}`);
      }
      ...
      if (!prtl.hasOwnProperty('content')) {
        ...
        if (typeof content !== 'string') {
          const errMsg = `Template partial "content" missing at index ${rtn[pnm].length} for ${JSON.stringify(prtl)}`
      ...
      rtn[pnm].push({ name: prtl.name, content, extension: prtl.extension, params });
    }
    return rtn;
  }
  ...
  async compile(name, template, extension) {
    ...
    if (!hasTmpl) { // partial template includes
      ...
      if (!ns.at.options.cacheRawTemplates || !ns.at.dbx[pnm][name] || !ns.at.dbx[pnm][name].hasOwnProperty('content')) {
      ...
      } else content = ns.at.dbx[pnm][name].content;
      // read/load rendering function?
      if (rd || !ns.at.options.cacheRawTemplates || !ns.at.dbx[snm][name] || !ns.at.dbx[snm][name].hasOwnProperty('func')) {
        const rd = await ns.this.read(name, false, extension);
        func = rd && rd.func;
      } else func = ns.at.dbx[snm][name].func;
    }
    if (!func) { // no rendering function requires a fresh compile
      func = Sandbox.compile(name, content, ns.at.dbx[pnm], ns.at.dbx[snm], ns.at.options, ns.this.readWriteName,
  ...
  async modularize(id, clear) {
    const ns = internal(this), snm = ns.at.options.sourceStoreName;
    // not really much to do here since modules/functions are created/stored in memory
    return ns.at.dbx[snm][id] && ns.at.dbx[snm][id].func;
  }

  /**
   * @returns {(Function | Function[])} One or more 
   ...
   * compilation cache (_on-the-fly_ loading). The functions should not reference any external scope other than the global object
   * space. When returning multiple functions, they should be in the order in which they will be executed.
   */
  get readers() {
    return fetcher;
  }
