...
// TODO : ESM use... export class Cachier {
   ...
   * Accepts the same arguments as `operations[].write` and all _scoped_ functions will be available. Can return a __rendering__
   * function that will prevent further iteration of any `operations[].write` invocations.
...
async function readAndSet(mods, name, path, store, forContent, extension, optional, params, close, registrant, log, isCompile) {
  ...
  if (log.info) {
    log.info(`${isFetch ? 'HTTPS' : 'MEMORY'}: Reading template ${forContent ? 'partial' : 'code'} `
...
async function writeAndSet(mods, name, path, optional, params, store, data, forContent, extension, registrant, log, isCompile) {
  ...
  if (!useCache) {
    if (log && log.info) {
      log.info(`MEMORY: Skipping write due to "options.cacheRawTemplates" is turned off for template "${name}" @ "${path}"${logLabel}`);
    }
    return;
  }
  data = data || '';
  ...
  if (log.info) {
    log.info(`${isFetch ? 'HTTPS' : 'MEMORY'}: Writting template ${forContent ? 'partial' : 'code'} `
