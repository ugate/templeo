 ...
 * @property {Boolean} [useCommonJs=false] When true, __CommonJS Module__ semantics will be used. When false, __ECMAScript Module__ semantics
 * will be used.
 ...
 * Depending upon the `Cachier` being used on the `Engine`, the options will be passed into the standard Fetch API or another cache-specific `read`
 ...
 * Depending upon the `Cachier` being used on the `Engine`, the options will be passed into the standard Fetch API or another cache-specific `write`
 * operation or simply ignored.
 ...
 * @property {Number} [maxCacheEntries=0] Maximum combined in-memory raw-template and compiled-renderer entries. `0` keeps the existing unlimited behavior. When the limit is exceeded, the least-recently-used entry is evicted.
 * @property {Number} [maxCacheBytes=0] Approximate maximum combined in-memory cache size in bytes. `0` keeps the existing unlimited behavior. When the limit is exceeded, least-recently-used entries are evicted.
 * @property {Number} [cacheTTL=0] Idle time-to-live in milliseconds for in-memory cache entries. `0` disables expiration. Each successful cache access refreshes the entry timestamp.
