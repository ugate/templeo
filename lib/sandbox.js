...
function coded(name, template, includes, compileOpts, directives, nameRegExp, namer, reader) {
  ...
  const incl = `var include; { ${namex}${readx} { ${nmreg}${coptx}${roptx}${inclsx}${varx}${dirsx}${sandx}include=${include.toString()} } }`;
...
async function include(strs, ...exps) {
  ...
  const read = async function read(name) { // read the render-time template content
    var content;
    try {
      content = await reader(namer(name, optional, true), optional, true);
    } catch (err) {
