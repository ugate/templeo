  ...
  static deserialzeBlock(block, name, isAsync) {
    ...
    if (block && (type === 'string' || type === 'function')) {
      ...
      try {
        ...
        err.message += ` <- Unable to deserialize code on name: "${name || ''}" for: ${code}`;
        throw err;
      }
    }
  }
...
function coded(name, template, includes, compileOpts, directives, nameRegExp, namers, operations, log, params, compileMeta, storeMeta, metadata) {
  ...
  return `${meta}${directives.code}; { ${incl}${ctx}${prm}${debug}const result=\`${template}\`;${fnshx}return result; }${srcurl}\n`;
}
 ...
 * argument on each reader. Once all read operations have been invoked without a __name__ argument, `storeMeta.hasReadAllOnInit` will be
 * set to __true__.
...
async function renderContent(name, path, ext, content, params) {
  ...
  if (ext && policy.includes('write')) {
    const isClosePolicy = policy.includes('close');
    let error, nerr, wcnt = 0;
    for (let op of operations) {
      wcnt++;
      if (!op.write) continue;
      try {
        await op.write(name, path.replace(new RegExp(`.${ext}$`, 'g'), '.js'), 'js', true, optional, params, store, renderer, writeFormatter, isClosePolicy, log);
      } catch (err) {
