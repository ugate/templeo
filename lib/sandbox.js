...
module.exports = class Sandbox {
   ...
   * @param {String} includes[].tmpl The partial template content
...
async function include(strs, ...exps) {
  ...
  for (let str of strs) {
    if (includes[str] && includes[str].hasOwnProperty('tmpl')) {
      try {
        prms.push(toContent(includes[str].name, includes[str].tmpl));
      } catch (err) {
  ...
  if (rds) {
    for (let read of rds) { // process the dynamic reads
      try {
        ...
        prms.push(toContent(read.name, read.content));
      } catch (err) {
