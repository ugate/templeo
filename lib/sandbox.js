...
class Sandbox {
   ...
   * @param {(Function | Function[] | Object | Object[])} [readers] One or more 
   ...
   * be executed. Each entry can also be an `{ read:Function, finish:Function }` where the reader is the aforementioned reader
   * function, __finish__ is an `async function(storage:Object)` that can perform cleanup tasks for each reader after rendering
   * has completed and __readAllOnInit__ is a flag that indicates `read` will be invoked without a _name_ parameter during
   * initialization. The __storage__ object can contain metadata for read operations and `storage.partials` should
   * contain each of the read paratial template metadata. Read functions can return the partial template content and/or it can be
   * set on the `storage.partials`.
...
function coded(name, template, includes, compileOpts, directives, nameRegExp, namer, readers, params, metadata) {
  ...
  const inclsx = `const optional=${optional.toString()};const includes=${JSON.stringify(includes)};const store={partials:includes};console.log('INITTTTTTTTTTTTTTTTTTTTTTTT', includes);`;
  ...
  const incl = `var include,ctx,_readHander; { ${namex}${readx} { ${nmreg}${coptx}${roptx}${inclsx}${varx}${dirsx}${sandx}${itx}${inclx} } }`;
 ...
 * that have `reader.readAllOnInit` that call `readers.read` without a name
 */
async function _readHander(isFinish) {
  if (metadata.parent) return;
  for (let rdr of readers) {
    if (isFinish) {
      if (rdr.finish) await rdr.finish(store);
    } else if (rdr.readAllOnInit) await rdr.read(null, optional, null, store);
  }
}
...
async function renderRead(name, forContext, sprms, iprms, fromExpression) {
  ...
  if (includes[fullName] && includes[fullName].hasOwnProperty('content')) {
    ...
    let error, rcnt = 0, named;
    for (let rdr of readers) {
      try {
        rcnt++;
        named = namer(name, optional, true, null, forContext, sprms);
        rtn = await rdr.read(named, optional, sprms, store, undefined, console);
        if (typeof rtn === 'string') break;
        else if (store.partials[named] && typeof store.partials[named].content === 'string') {
          rtn = store.partials[named].content;
          break;
        }
