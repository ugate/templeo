...
class Sandbox {
   ...
   * be executed. Each entry can also be an `{ read:Function, finish:Function, readAllOnInitWhenEmpty:Boolean }` where `read` is
   * the aforementioned reader function, __finish__ is an `async function(storage:Object)` that can perform cleanup tasks for
   * each reader after rendering has completed and __readAllOnInitWhenEmpty__ is a flag that indicates `read` will be invoked
   * without a _name_ parameter when no partials exist during initialization. The __storage__ object can contain metadata for
   * read operations and `storage.partials` should contain each of the read paratial template metadata. Read functions can return
   * the partial template content and/or it can be set on the `storage.partials`.
...
async function _readHander(isFinish) {
  ...
  for (let rdr of readers) {
    if (isFinish) {
      if (rdr.finish) await rdr.finish(store, log);
    } else if (rdr.readAllOnInitWhenEmpty && !storeMeta.hasReadAll && !Object.getOwnPropertyNames(includes).length) {
      await rdr.read(null, optional, null, store, undefined, log);
      storeMeta.hasReadAll = true;
    }
  }
  if (isFinish) {
...
async function renderRead(name, forContext, sprms, iprms, fromExpression) {
  ...
  if (includes[fullName] && includes[fullName].hasOwnProperty('content')) {
    ...
    for (let rdr of readers) {
      try {
        rcnt++;
        rtn = await rdr.read(fullName, optional, sprms, store, undefined, log);
