...
class Sandbox {
   ...
   */
  static compile(name, template, includes, includeFuncs, compileOpts, namers, readers, director, logging) {
 ...
 */
function coded(name, template, includes, compileOpts, directives, nameRegExp, namers, readers, logging, params, metadata, storeMeta) {
  ...
  namex += ']);'
  var rdrs = Array.isArray(readers) ? readers : [readers], readerAdded, rdro, readx = 'const readers=Object.freeze([';
  for (let rdr of rdrs) { 
    rdro = typeof rdr === 'function' ? `{read:${rdr.toString()}}` : rdr && typeof rdr.read === 'function' ?
      ...
      + `,readAllOnInitWhenEmpty:${!!rdr.readAllOnInitWhenEmpty}` : ''}}` : null;
    if (rdro) {
