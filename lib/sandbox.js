...
class Sandbox {
   ...
   * @param {Function} [namers.namerOnReadError]  The naming function to use when a `readers[].read` function throws an error. The next
   * reader called in the `readers[]` list will use the name generated by this reader.
   ...
   * @param {Function} [readers[].start] An `async function` that can perform initialization tasks for a reader
   ...
   * @param {Function} [readers[].finish] An `async function` that can perform cleanup tasks for a reader
...
function coded(name, template, includes, compileOpts, directives, nameRegExp, namers, readers, log, params, compileMeta, storeMeta, metadata) {
  ...
  let nmrs = typeof namers === 'object' ? namers : {}, namersAdded, nmro, namex = 'const namers=Object.freeze({';
  for (let nmr in nmrs) {
    if (!nmrs.hasOwnProperty(nmr)) continue;
    nmro = typeof nmrs[nmr] === 'function' ? nmrs[nmr].toString() : null;
    if (nmro) {
      namex += `${namersAdded ? ',' : ''}${nmr}:${nmro}`;
      namersAdded = true;
    }
  }
  namex += '});';
  ...
  const inclx = `_readHander=${_readHander.toString()};include=${include.toString()};if (await _readHander()) { return ''; };`;
...
async function _readHander(isFinish) {
  ...
  for (let rdr of readers) {
    if (isFinish) {
      ...
      if (rdr.start && (await rdr.start(store, optional, log)) === true) return true;
 ...
 * - `readers` - One or more async functions that will read partial templates
 * - `namers` - An object that contains one or more functions that will compose names that will be passed into the readers
  ...
  if (includes[fullName] && includes[fullName].hasOwnProperty('content')) {
    ...
    for (let rdr of readers) {
      try {
        ...
        if (typeof namers.namerOnReadError === 'function') {
          try {
            fullName = await namers.namerOnReadError(namers, name, optional, sprms, store, true, null, forContext, fullName);
            ...
            + ` READ ERROR NAMER: "${namers.namerOnReadError.name}" for include template @ "${fullName || name}" (render-time read)`);
