...
module.exports = class Sandbox {
   ...
   * `async function(partialName:String, optional:(TemplateOpts | Function(name:String):*), params:Object):String`
   ...
   * @param {typeof Director} [DirClass] The {@link Director} class that will be for extracting {@link Director.directives}
 ...
 * found within `includes`. When `compileOpts.isCached` is _truthy_ an attempt will be made to update `includes` with
 * the template name/content to prevent unnecessary template partial reads.
 * @param {Object} [params] The key/value parameters to pass into the `reader` and will be available as
 * `options.includesParametersName` within the scope of the included template
 ...
 */
function coded(name, template, includes, compileOpts, directives, nameRegExp, namer, reader, params) {
  ...
  // directives
  return `${directives.code}; { ${incl}${ctx}${prm} return \`${template}\`; }\n//# sourceURL=${name}.js\n`;
}
...
function renderContent(name, content, params) {
  ...
  const block = coded(name, content, includes, compileOpts, directives, nameRegExp, namer, reader, params);
 ...
 */
async function renderRead(name, forContext, params) {
  var rtn;
  try {
    rtn = await reader(namer(name, optional, true, null, forContext), optional, params);
  } catch (err) {
  ...
  if (forContext) {
    ...
    try {
      rtn = await renderContent(name, rtn, params);
      if (optional('isCached')) {
        includes[name] = { name, content: rtn, [optional('includesParametersName') || 'params']: params, renderTime: true };
      }
    } catch (err) {
...
async function include(strs, ...exps) {
  var rtn = '';
  for (let i = 0, ln = Math.max(strs.length, exps.length), str, exp, names, ni, prms; i < ln; i++) {
    str = strs[i];
    exp = exps[i];
    if (str && exp && typeof exp === 'object') { // expression has read parameters
      if (reader && optional('renderTimeIncludeReads')) {
        for (let name in exp) {
          if (!exp.hasOwnProperty(name)) continue;
          prms = prms || {};
          prms[name] = exp[name];
        }
      }
    ...
    if (names) {
      ...
      for (let name of names) {
        ni++;
        if (includes[name] && includes[name].hasOwnProperty('content')) {
          try {
            rtn += await renderContent(name, includes[name].content, includes[name].params);
          } catch (err) {
            err.message += ` <- Unable to include template @ ${name} (${ni > 0 ? 'expression' : 'string'} literal)`;
            throw err;
          }
        } else if (reader && optional('renderTimeIncludeReads')) {
          rtn += await renderRead(name, false, prms);
        } else throw new Error(`Cannot find included template @ ${name} (${ni > 0 ? 'expression' : 'string'} literal)`);
      }
    }
    prms = null; // parameters do not carry over from one string/expression to the next
  }
  return rtn;
}