...
class Sandbox {
   ...
   * @param {Function} [namers.namerOnReadError]  The naming function to use when a `operations[].read` function throws an error. The next
...
function coded(name, template, includes, compileOpts, directives, nameRegExp, namers, operations, log, params, compileMeta, storeMeta, metadata) {
  ...
  const addOp = (op, idx) => {
    ...
    if (Array.isArray(op.scopes) && op.scopes.length) {
      opo = `readWrite:function(opFunc, ...args){`;
      for (let sfunc of op.scopes) {
        if (typeof sfunc === 'function' && sfunc.name) {
          opo += `let ${sfunc.name}=${sfunc.toString()};`;
        }
      }
      opo += `return opFunc.apply(undefined, args);}`;
      opo += `,get read(){let thiz=this;return function ${rdf.name}(){return thiz.readWrite(${rdf.toString()},...arguments);}}`;
      if (wrf) {
        opo += `,get write(){let thiz=this;return function ${wrf.name}(){return thiz.readWrite(${wrf.toString()},...arguments);}}`;
      }
    } else {
...
async function renderRead(name, forContext, sprms, iprms, fromExpression) {
  ...
  if (includes[fullName] && includes[fullName].hasOwnProperty('content')) {
  ...
  } else if (operations.length && optional('renderTimeReadPolicy') !== 'none') { 
    ...
    const cause = operations.length ? '"options.renderTimeReadPolicy" is set to "none"' : 'no reader function(s) have been defined';
...
async function include(strs, ...exps) {
  ...
  for (let i = 0, ln = Math.max(strs.length, exps.length), str, exp, names, ni, sprms, iprms; i < ln; i++) {
    ...
    if (str && exp instanceof URLSearchParams) {
      if (operations.length && optional('renderTimeReadPolicy') !== 'none') sprms = exp;
      exp = null;
