...
function coded(name, template, includes, compileOpts, directives, nameRegExp, namers, operations, log, params, compileMeta, storeMeta, metadata) {
  ...
  const addOp = (op, idx) => {
    ...
    if (scp) {
      ...
      opo += `scopedOp:function(type,thiz,args){${opScoped} if(typeof type === 'undefined'){return;} const _op=type === 1 ? ${rdf.toString()} : type === 2 ? ${wrf.toString()} : ${fsh ? fsh.toString() : '()=>null'};`;
      ...
      for (let onm of opNames) {
        ...
        opo += `return thiz.scopedOp(${onm === 'read' ? 1 : 2},thiz,arguments);};`;
        opo += `${onm}.toString=() => ${onm}().toString();`;
        opo += `return ${onm};}`;
      }
      if (fsh) opo += `,get finish(){const thiz=this;const scopedFinish=function finish(){return thiz.scopedOp(0,thiz,arguments);};}`;
      opo += `,scopes:[]`;
    } else {
