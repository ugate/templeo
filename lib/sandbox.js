...
class Sandbox {
  ...
  * @param {Function} [operations[].write] The write function that will be used for writting newly discovered templates and/or sources.
...
function coded(name, template, includes, compileOpts, directives, nameRegExp, namers, operations, log, params, compileMeta, storeMeta, metadata) {
  ...
  const addOp = (op, idx) => {
    ...
    const wrf = typeof op.write === 'function' ? op.write : function noopWrite() { Promise.resolve() };
    const srt = typeof op.start === 'function' ? op.start : null;
    ...
    if (scp) {
      ...
      if (srt) opNames.push({ name: 'start', func: srt, num: 0 });
      ...
      opo += `const _op=type === 1 ? ${rdf.toString()} : type === 2 ? ${wrf.toString()} : type === 0 ? ${srt ? srt.toString() : '()=>null'} : ${fsh ? fsh.toString() : '()=>null'};`;
  ...
  const incl = `var include,ctx,_readHandler; { ${logx}${namex}${opx} { ${nmreg}${coptx}${roptx}${frmtx}${inclsx}${storex}${varx}${dirsx}${sandx}${inclx}${itx} } }`;
  ...
  for (let op of operations) {
    if (isFinish) {
      ...
      if (!storeMeta.hasReadAllOnInit) {
        try {
          await op.read(null, null, null, true, optional, null, store, readFormatter, true, log);
        } catch (err) {
 ...
 */
async function renderContent(name, path, ext, content, params) {
  if (!content) content = optional('defaultPartialContent');
  // new Function restricts access to closures (other than globals)
  const block = coded(path, content, includes, compileOpts, directives, nameRegExp, namers, operations,
  ...
  if (policy.includes('write')) {
    const isClosePolicy = policy.includes('close');
    let error, nerr, wcnt;
    for (let op of operations) {
      wcnt++;
      if (!op.write) continue;
      try {
        await op.write(name, path, ext, true, optional, params, store, content, writeFormatter, isClosePolicy, log);
      } catch (err) {
...
async function renderRead(name, forContext, sprms, iprms, fromExpression) {
  ...
  if (includes[path] && includes[path].hasOwnProperty('content')) {
    if (log && log.info) log.info(`Render READ: Found cached "${path}"`);
    rtn = includes[path].content;
    ...
    for (let op of operations) {
      try {
        ...
        } else if (store.partials[path]) {
          rtnType = store.partials[path].content && typeof store.partials[path].content;
          if (rtnType === 'string' || rtnType === 'object') {
            rtn = store.partials[path].content;
            break;
          }
        }
        ...
        if (typeof namers.namerSuper === 'function') {
          try {
            ...
            if (store.partials[path] && typeof store.partials[path].content === 'string') {
              rtn = store.partials[path].content;
              break;
            }
          } catch (nerr) {
        ...
        + ` reader names: "${fnames.join('" >> "')}" using policy "${policy}"${err instanceof TypeError ? 
          ' Ensure that a proper *PathBase option has been set' : ''} (render-time read)`
    ...
    if (typeof rtn === 'string' && optional('cacheRawTemplates')) {
      includes[path] = { name: path, shortName: name, content: rtn, renderTime: true };
    }
  } else if (sprms) {
  ...
  if (forContext) {
    ...
    try {
      rtn = await renderContent(name, path, ext, rtn, iprms);
    } catch (err) {
