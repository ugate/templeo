...
function coded(name, template, includes, compileOpts, directives, nameRegExp, namers, operations, log, params, compileMeta, storeMeta, metadata) {
  ...
  const addOp = (op, idx) => {
    ...
    const scp = op && Array.isArray(op.scopes) && op.scopes.length ? op.scopes : null, opNames = ['read', 'write'];
    let opo = '', opScoped = '';
    if (scp) {
      ...
      opo += `readWrite:function(isRead,thiz,args){${opScoped} if(typeof isRead === 'undefined'){return;} const _op=isRead ? ${rdf.toString()} : ${wrf.toString()};`;
      opo += `return args && args.length ? _op.apply(undefined, args) : _op;}`;
      for (let onm of opNames) {
        opo += `,get ${onm}(){const thiz=this;const ${onm}=function ${onm === 'read' ? rdf.name : wrf.name}(){`;
        opo += `return thiz.readWrite(${onm === 'read'},thiz,arguments);};`;
        opo += `${onm}.toString=() => ${onm}().toString();`;
        opo += `return ${onm};}`;
      }
      opo += `,scopes:[]`;
    } else {
  ...
  for (let op of operations) {
    if (isFinish) {
      ...
      if (!storeMeta.hasReadAllOnInit) {
        try {
          await op.read(null, null, null, optional, null, store, readFormatter, true, log);
        } catch (err) {
 ...
 */
function renderContent(name, path, content, params) {
  ...
  if (policy.includes('write')) {
    /*for (let op of operations) {
      ...
      renderer = await op.write(name, path, optional, params, store, block, writeFormatter, policy.includes('close'), log);
      if (typeof renderer === 'function') break;
    }*/
  }
  if (typeof renderer !== 'function') {
    ...
    renderer = new Function(`${rqrd}return async function ${named}(){ ${block}; }`)(rqr);
  }
...
async function renderRead(name, forContext, sprms, iprms, fromExpression) {
  ...
  if (includes[path] && includes[path].hasOwnProperty('content')) {
    ...
    let error, nerr, rcnt = 0, close, fnames, policy = optional('renderTimePolicy');
    for (let op of operations) {
      try {
        rcnt++;
        let ext = path && path.match(/^.*?\.([^/.]+)$/);
        ext = ext && ext[1];
        close = policy.includes('close');
        rtn = await op.read(name, path, ext ? ext : undefined, optional, sprms, store, readFormatter, close, log);
        if (typeof rtn === 'string') break;
        else if (store.partials[path] && typeof store.partials[path].content === 'string') {
  ...
  if (forContext) {
    ...
    try {
      rtn = await renderContent(name, path, rtn, iprms);
    } catch (err) {
