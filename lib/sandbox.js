...
module.exports = class Sandbox {
   ...
   */
  static renderer(name, template, includes, compileOpts, reader) {
    ...
    const code = coded(template, includes, copts, reader, directives);
...
async function include(strs, ...exps) {
  ...
  const toContent = content => {
    ...
    // eval is preferred over new Function here since include should already sandboxed and access to closures is required
    const rtn = eval(`\`${content}\``);
    //const rtn = (new Function(coded(content, includes, compileOpts, reader, directives)))(context, renderOpts);
    return rtn;
  };
  const read = async name => { // read the render-time template content
    try {
      const content = toContent(await reader(name));
  ...
  for (let str of strs) {
    if (includes[str] && includes[str].hasOwnProperty('content')) {
      try {
        rtn += toContent(includes[str].content);
      } catch (err) {
  ...
  for (let exp of exps) {
    if (includes[exp] && includes[exp].hasOwnProperty('content')) {
      try {
        rtn += toContent(includes[exp].content);
      } catch (err) {
