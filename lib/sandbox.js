...
class Sandbox {
   ...
   * @param {(Function | Function[])} [namers] One or more async functions responsible for formatting template names into a full path name
   * that is consumable by `read`/`write`. Each function will be executed in order- passing the previously returned name into the next
   * naming function. The following arguments will be passed:
   ...
   * 1. _{String}_ `named` The returned string from a previously invoked namer function. This allows naming conventions to be chained in
   * order to compose a final name.
   ...
   * @param {Function} [readers[].start] An `async function(storage:Object, optional:(TemplateOpts | Function(name:String):*), log:Object)`
   * that can perform initialization tasks for a reader __before rendering has started__
   * @param {Function} [readers[].finish] An `async function(storage:Object, optional:(TemplateOpts | Function(name:String):*), log:Object)`
   * that can perform cleanup tasks for a reader __after rendering has completed__
   * @param {Function[]} [readers[].scopes] Zero or more functions that will be in scope when the read function is called.
   * Scoped functions can assit with complex readers that can benefit from separate supporting functions. For example, `[myFunc(){}]`
   ...
   */
  static compile(name, template, includes, includeFuncs, compileOpts, namers, readers, director, compileMeta, log) {
    ...
    const code = coded(named, template, includes, copts, directives, FUNC_NAME_REGEXP, namers, readers, {
 ...
 * @param {(Function | Function[])} [namers] One or more functions responsible for formatting template names into a full path name
 * that is consumable by `read`/`write`. Each function will be executed in order- passing the previously returned name into the next
 * naming function. The following arguments will be passed:
 * 1. _{String}_ `partialName` The name of the partial that will be converted into a name suitable for a read operation.
 * 1. _{(TemplateOpts | Function(name:String):*)}_ `optional` Either the {@link TemplateOpts} or a function that takes a single name
 ...
 */
function coded(name, template, includes, compileOpts, directives, nameRegExp, namers, readers, log, params, compileMeta, storeMeta, metadata) {
  ...
  const logx = `const log={debug:${log && log.debug ? typeof log.debug === 'function' ? log.debug.toString() : 'console.debug' : false}`
    + `,info:${log && log.info ? typeof log.info === 'function' ? log.info.toString() : 'console.info' : false}`
    + `,warn:${log && log.warn ? typeof log.warn === 'function' ? log.warn.toString() : 'console.warn' : false}`
    + `,error:${log && log.error ? typeof log.error === 'function' ? log.error.toString() : 'console.error' : false}};`;
  var nmrs = Array.isArray(namers) ? namers : [namers], namersAdded, nmro, namex = 'const namers=Object.freeze([';
  for (let nmr of nmrs) {
    nmro = typeof nmr === 'function' ? nmr.toString() : null;
    if (nmro) {
      namex += `${namersAdded ? ',' : ''}${nmro}`;
      namersAdded = true;
    }
  }
  namex += ']);';
  var rdrs = Array.isArray(readers) ? readers : [readers], readerAdded, rfunc, rdro, readx = 'const readers=Object.freeze([';
  for (let rdr of rdrs) {
    rfunc = typeof rdr === 'function' ? rdr : rdr && typeof rdr.read === 'function' ? rdr.read : null;
    if (rfunc) {
      if (Array.isArray(rdr.scopes) && rdr.scopes.length) {
        rdro = `read:function ${rdr.name || rdr.read.name}(){`;
        for (let func of rdr.scopes) {
          if (typeof func === 'function' && func.name) {
            rdro += `let ${func.name}=${func.toString()};`;
          }
        }
        rdro += `return (${rfunc.toString()}).apply(undefined, arguments);}`;
      } else rdro = `read:${rfunc.toString()}`;
      if (typeof rdr.finish === 'function') {
        rdro += `,finish:${rdr.finish.toString()}`;
      }
      rdro += `,readAllOnInitWhenEmpty:${!!rdr.readAllOnInitWhenEmpty}`;
      readx += `${readerAdded ? ',' : ''}Object.freeze({${rdro}})`;
      readerAdded = true;
    }
  }
  readx += ']);';
  ...
  const storex = `const compileMeta=${compileMeta ? JSON.stringify(compileMeta) : '{}'};const store={partials:includes,compileMeta};`
    + `const storeMeta=${storeMeta ? JSON.stringify(storeMeta) : '{}'};`;
 ...
 * as well as calling `reader.read` without a name argument on any readers that have have a _truthy_ value for
 * `reader.readAllOnInitWhenEmpty`
 */
async function _readHander(isFinish) {
  ...
  for (let rdr of readers) {
    if (isFinish) {
      ...
      if (rdr.start) await rdr.start(store, optional, log);
      if (!storeMeta.hasReadAllOnInit && optional('renderTimeReadPolicy') === 'read-all-on-init-when-empty'
 ...
 * - `readers` - One or more async functions that will read partial templates
 * - `namers` - One or more functions that will compose names that will be passed into the readers
...
function renderContent(name, content, params) {
  ...
  const block = coded(name, content, includes, compileOpts, directives, nameRegExp, namers, readers,
...
async function renderRead(name, forContext, sprms, iprms, fromExpression) {
  let rtn, fullName, ncnt = 0;;
  for (let namer of namers) {
    try {
      ncnt++;
      fullName = await namer(name, optional, sprms, store, true, null, forContext, fullName);
    } catch (err) {
      err.message += `${err.message} <- Unable to extract a name for con${forContext ? 'text' : 'tent'} via`
      + ` NAMER #${ncnt}: "${namer.name}" for include template @ "${fullName}" (render-time read)`;
      throw err;
    }
  }
  if (includes[fullName] && includes[fullName].hasOwnProperty('content')) {
    ...
    for (let rdr of readers) {
      try {
        ...
        throw new Error(`Reader "${rdr.read.name}" returned "${typeof rtn}" rather than an expected content "string"`);
      } catch (err) {
