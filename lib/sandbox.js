...
class Sandbox {
  ...
  static deserialzeBlock(block, name, isAsync) {
    ...
    if (block && (type === 'string' || type === 'function')) {
      ...
      // expose native dynamic import within the generated function so serialized renderers retain it
      const imprtd = `const importModule=specifier=>import(specifier);`;
      const code = `${rqrd}return ${isAsync ? 'async ' : ''}function${named}(){ ${imprtd}${block}; }`;
      try {
...
function coded(name, template, compileOpts, directives, nameRegExp, namers, operations, log, params, store, storeMeta, metadata) {
  ...
  const storedx = `if(storeDta && storeDta !== store.data){for (let nm in storeDta){if(Object.prototype.hasOwnProperty.call(store.data,nm))continue;store.data[nm]=storeDta[nm];}};`;
...
async function renderContent(name, path, ext, content, params, fromCache) {
  ...
  const rqrd = rqr ? `const require=arguments[0];` : '';
  // expose native dynamic import within the generated partial so serialized renderers retain it
  const imprtd = `const importModule=specifier=>import(specifier);`;
  const named = path.replace(nameRegExp, '_');
  let renderer = new Function(`${rqrd}return async function ${named}(){ ${imprtd}${block}; }`)(rqr);
  if (!fromCache && ext) {
    const policy = optional('renderTimePolicy');
    if (policy.includes('write')) {
      const isClosePolicy = policy.includes('close'), moduleExt = optional('useCommonJs') ? 'cjs' : 'mjs';
      const pathValue = String(path), queryIndex = pathValue.indexOf('?'), hashIndex = pathValue.indexOf('#');
      let suffixIndex = pathValue.length;
      if (queryIndex >= 0) suffixIndex = Math.min(suffixIndex, queryIndex);
      if (hashIndex >= 0) suffixIndex = Math.min(suffixIndex, hashIndex);
      const pathBase = pathValue.slice(0, suffixIndex), suffix = pathValue.slice(suffixIndex);
      const contentExt = ext ? `.${String(ext).replace(/^\./, '')}` : '';
      const sourceBase = contentExt && pathBase.toLowerCase().endsWith(contentExt.toLowerCase())
        ? pathBase.slice(0, -contentExt.length) : pathBase;
      const srcPath = `${sourceBase}.${moduleExt}${suffix}`;
      let error, nerr, wcnt = 0, writeRenderer;
      for (let op of operations) {
        wcnt++;
        if (!op.write) continue;
        try {
          writeRenderer = await op.write(name, srcPath, moduleExt, false, optional, params, store, renderer, writeFormatter, isClosePolicy, log);
...
async function renderRead(name, forContext, sprms, iprms, fromExpression) {
  ...
  const extensionPath = path && String(path).split(/[?#]/, 1)[0];
  let ext = extensionPath && extensionPath.match(/^.*?\.([^/.]+)$/), fromCache;
  ext = ext && ext[1];
