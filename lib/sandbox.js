...
class Sandbox {
   ...
   * be executed. Each entry can also be an `{ reader:Function, finish:Function }` where the reader is the aforementioned reader
   * function and __finish__ is an `async function(storage:Object)` that can perform cleanup tasks for each reader after
   * rendering has completed. The __storage__ object can contain metadata for read operations and `storage.partials` should
   * contain each of the read paratial template metadata.
...
function coded(name, template, includes, compileOpts, directives, nameRegExp, namer, readers, params, metadata) {
  ...
  for (let rdr of rdrs) { 
    rdro = typeof rdr === 'function' ? `{reader:${rdr.toString()}}` : rdr && typeof rdr.reader === 'function' ?
      `{reader:${rdr.reader.toString()}${typeof rdr.finish === 'function' ? `,finish:${rdr.finish.toString()}` : ''}}` : '';
    if (rdro) {
  ...
  const inclsx = `const optional=${optional.toString()};const includes=${JSON.stringify(includes)};const store={partials:includes};`;
  ...
  const inclx = `_finish=${_finish.toString()};include=${include.toString()};`;
  ...
  const incl = `var include,ctx,_finish; { ${namex}${readx} { ${nmreg}${coptx}${roptx}${inclsx}${varx}${dirsx}${sandx}${itx}${inclx} } }`;
  const fnsh = `await _finish();`;
  ...
  if (includes[fullName] && includes[fullName].hasOwnProperty('content')) {
    ...
    for (let rdr of readers) {
      try {
        ...
        rtn = await rdr.reader(named, optional, sprms, store, undefined, console);
        ...
        error = new Error(`${err.message} <- ${error ? ` <- ${error.message}` : ''} <- Unable to read content via "${rdr.reader.name}" for include template @`
