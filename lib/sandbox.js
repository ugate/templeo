...
function coded(name, template, includes, compileOpts, directives, nameRegExp, namers, operations, log, params, compileMeta, storeMeta, metadata) {
  ...
  namex += '});';
  let ops = Array.isArray(operations) ? operations : [operations], opAdded, opx = 'debugger;const operations=Object.freeze([';
  const addOp = (op, idx) => {
    ...
    let opo = '', opScoped = '', opScopes = '';
    if (scp) {
      for (let sfunc of scp) {
        if (typeof sfunc === 'function' && sfunc.name) {
          opScoped += `let ${sfunc.name};for (let func of thiz.scopes){if (func.name === '${sfunc.name}'){${sfunc.name}=func;break;}}`;
          opScopes += `${opScopes ? ',' : ''}${sfunc.toString()}`;
        }
      }
      for (let onm of opNames) {
        if (onm !== 'read') opo += ',';
        opo += `get ${onm}(){const thiz=this;let ${onm}Raw;const ${onm}=function ${rdf.name}(){${opScoped}`;
        opo += `${onm}Raw=${rdf.toString()};return arguments.length ? ${onm}Raw.apply(undefined, arguments) : ${onm}Raw;};`;
        opo += `${onm}.toString=() => ${onm}Raw ? ${onm}Raw.toString() : ${onm}().toString();`;
        opo += `return ${onm};}`;
      }
      opo += `,scopes:Object.freeze([${opScopes}])`;
    } else {
  ...
  for (let op of operations) {
    if (isFinish) {
      ...
      if (!storeMeta.hasReadAllOnInit) {
        try {
          await op.read(null, optional, null, store, readFormatter, true, log);
        } catch (err) {
...
function renderContent(name, content, params) {
  ...
  if (policy.includes('write')) {
    /*for (let op of operations) {
      ...
      renderer = await op.write(name, optional, params, store, block, writeFormatter, policy.includes('close'), log);
  ...
  if (includes[fullName] && includes[fullName].hasOwnProperty('content')) {
    ...
    for (let op of operations) {
      try {
        ...
        rtn = await op.read(fullName, optional, sprms, store, readFormatter, close, log);
