...
function coded(name, template, includes, compileOpts, directives, nameRegExp, namers, operations, log, params, compileMeta, storeMeta, metadata) {
  ...
  const addOp = (op, idx) => {
    ...
    if (Array.isArray(op.scopes) && op.scopes.length) {
      ...
      opo += `,get read(){let thiz=this;return function ${rdf.name}(){return thiz.readWrite(true,...arguments);}}`;
      if (wrf) {
        opo += `,get write(){let thiz=this;return function ${wrf.name}(){return thiz.readWrite(false,...arguments);}}`;
      }
    } else {
