...
class Sandbox {
   ...
   * render-time reads
   * @param {Function} [readers[].read] The reader
...
async function _readHander(isFinish) {
  ...
  for (let rdr of readers) {
    if (isFinish) {
      ...
      await rdr.read(null, optional, null, store, true, undefined, log);
...
async function renderRead(name, forContext, sprms, iprms, fromExpression) {
  ...
  if (includes[fullName] && includes[fullName].hasOwnProperty('content')) {
    ...
    let error, rcnt = 0, close;
    for (let rdr of readers) {
      try {
        rcnt++;
        close = optional('renderReadPolicy') === 'read-and-close';
        rtn = await rdr.read(fullName, optional, sprms, store, close, undefined, log);
