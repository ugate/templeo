...
module.exports = class Sandbox {
  ...
  /**
   * Generates a locally sandboxed `async` template rendering function
   ...
   * @param {Function} [reader] A reader `function(partialName:String):String` that returns a partial template
   * content by name when it is not present in `includes` for an _on-the-fly_ loading of partials.
   * @returns {Function} An `async` function that wi 
   */
  static renderer(name, template, includes, includeFuncs, compileOpts, reader) {
    ...
    const code = coded(named, template, includes, copts, reader, directives);
 ...
 */
function coded(name, template, includes, compileOpts, reader, directives) {
...
async function include(strs, ...exps) {
  ...
  const toContent = (name, content) => {
    ...
    const block = coded(named, content, includes, compileOpts, reader, directives);
    if (!content) content = renderOpts.defaultPartialContent;
  ...
  const read = async name => { // read the render-time template content
    try {
      const content = await toContent(name, await reader(name));
      if (compileOpts.isCached) includes[name] = { name, content };
      return content;
    } catch (err) {
      err.message += ` CAUSE: Unable to include template @ ${name} (dynamic read)`;
      throw err;
    }
  };
  for (let str of strs) {
