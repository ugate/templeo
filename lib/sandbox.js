...
module.exports = class Sandbox {
   ...
   */
  static compile(name, template, includes, includeFuncs, compileOpts, namer, reader, director) {
 ...
 * __content__
 * @param {URLSearchParams} [sprms] The key/value parameters to pass into the `reader`
...
async function renderRead(name, forContext, sprms, iprms, fromExpression) {
  ...
  if (includes[fullName] && includes[fullName].hasOwnProperty('content')) {
    ...
    try {
      rtn = await reader(namer(name, optional, true, null, forContext, sprms), optional, sprms);
      if (optional('cacheRawTemplates')) {
        includes[fullName] = { name: fullName, content: rtn, renderTime: true };
      }
    } catch (err) {
      err.message += ` <- Unable to read content for include template @ ${fullName} (render-time read)`;
      throw err;
    }
  } else if (sprms) {
