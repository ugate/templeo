 ...
 * @param {Boolean} [storeMeta.hasReadAllOnInit] `true` when the templates have been initialized meetting the criteria described in {@link _readHander}
...
function coded(name, template, includes, compileOpts, directives, nameRegExp, namers, readers, log, params, compileMeta, storeMeta, metadata) {
  ...
  const incl = `var include,ctx,_readHander; { ${logx}${namex}${readx} { ${nmreg}${coptx}${roptx}${inclsx}${storex}${varx}${dirsx}${sandx}${inclx}${itx} } }`;
 ...
 * as well as calling `reader.read` without a __name__ argument on each reader. Once all readers have been invoked without a __name__
    ...
    if (isFinish) {
      ...
      if (!storeMeta.hasReadAllOnInit) {
        try {
          ...
          err.message += ` <- Failed to initiate reader "${rdr.read.name}"`;
          throw err;
        }
      }
    }
