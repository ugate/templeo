...
module.exports = class Sandbox {
   ...
   * `function(partialName:String, compileOptions:TemplateOpts, renderOptions:TemplateOpts, forContent:Boolean):String`
   ...
   * responsible for reading partial template content/modules/etc. when a template cannot be found within `includes`
   * (_on-the-fly_ loading)
   * @returns {Function} An `async` function that wi 
   */
  static compile(name, template, includes, includeFuncs, compileOpts, namer, reader) {
    ...
    const named = (name && name.replace(/\\|\/|\./g, '_')) || `template_${Sandbox.guid(null, false)}`;
    const directives = codedDirectives(copts.directives);
    const code = coded(named, template, includes, copts, directives, namer, reader);
  ...
  static deserialzeFunction(functionString) {
    if (functionString && typeof functionString === 'string') {
      try {
        ...
        err.message += ` CAUSE: Unable to deserialize function string: ${functionString}`;
        throw err;
      }
    }
  }
  ...
  static deserialzeBlock(block, name, isAsync) {
    if (block && typeof block === 'string') {
      const named = name && name.length ? ` ${name.replace(/\\|\/|\./g, '_')}` : '';
      ...
      try {
        ...
        err.message += ` CAUSE: Unable to deserialize code ${name || ''} for: ${code}`;
        throw err;
      }
    }
  }
 ...
 */
function coded(name, template, includes, compileOpts, directives, namer, reader) {
  ...
  const incl = `var include; { ${namex}${readx} { ${coptx}${roptx}${inclsx}${varx}${dirsx}${sandx}include=${include.toString()} } }`;
...
/**
 * Returns either the `renderOpts` or the `compileOpts` option value (in that order of precedence).
...
function optional(name) {
  return renderOpts.hasOwnProperty(name) ? renderOpts[name] : compileOpts[name];
}
 ...
 * - `optional` - The option function that extracts from either the `renderOpts` or `compileOpts` (in that order of precedence)
...
async function include(strs, ...exps) {
  var rtn = '';
  const toContent = (name, content) => {
    // new Function restricts access to closures (other than globals)
    //eval(`\`${content}\``)
    const named = name.replace(/\\|\/|\./g, '_');
    const block = coded(named, content, includes, compileOpts, directives, namer, reader);
  ...
  const read = async function read(name) { // read the render-time template content
    var content;
    try {
      content = await reader(namer(name, compileOpts, renderOpts, true), compileOpts, renderOpts, true);
    } catch (err) {
      err.message += ` CAUSE: Unable to read content for include template @ ${name} (render-time read)`;
      throw err;
    }
    try {
      content = await toContent(name, content);
      if (optional('isCached')) includes[name] = { name, content };
      return content;
    } catch (err) {
      err.message += ` CAUSE: Unable to include template @ ${name} (render-time read)`;
      throw err;
    }
  };
  for (let str of strs) {
    if (includes[str] && includes[str].hasOwnProperty('content')) {
      try {
        rtn += await toContent(str, includes[str].content);
      } catch (err) {
        err.message += ` CAUSE: Unable to include template @ ${str} (string literal)`;
        throw err;
      }
  ...
  for (let exp of exps) {
    if (includes[exp] && includes[exp].hasOwnProperty('content')) {
      try {
        rtn += await toContent(exp, includes[exp].content);
      } catch (err) {
        err.message += ` CAUSE: Unable to include template @ ${exp} (expression literal)`;
        throw err;
      }
    } else if (reader && optional('renderTimeIncludeReads')) {
      rtn += await read(exp);
