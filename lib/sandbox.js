...
module.exports = class Sandbox {
   ...
   */
  static compile(name, template, includes, includeFuncs, compileOpts, reader) {
    ...
    const code = coded(named, template, includes, copts, directives, reader);
  ...
  static deserialzeBlock(block, name, isAsync) {
    if (block && typeof block === 'string') {
      ...
      // pass require since it may be hidden from the block scope
 ...
 */
function coded(name, template, includes, compileOpts, directives, reader) {
...
async function include(strs, exps) {
  ...
  const toContent = (name, content) => {
    ...
    const block = coded(named, content, includes, compileOpts, directives, reader);
    if (!content) content = optional('defaultPartialContent');
    const renderer = new Function(`return async function ${named}(){ ${block}; }`)();
    return renderer(context, renderOpts);
  };
  const read = async function read(name) { // read the render-time template content
    var content;
    try {
      content = await reader(name, compileOpts, renderOpts, true);
    } catch (err) {
