 ...
 * rendering invocation. The read function will be invoked without a _name_ parameter when no partials exist during initialization. If partial templates
 * are present/registered during compilation, no DB reads will be made during rendering. 
 * - `read-and-close` Like the __read__ policy, partial templates are only read when include directives are encountered. The DB will be opened before
 * each read and closed after each read has completed for a given partial.
 */
