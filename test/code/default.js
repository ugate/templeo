...
class Tester {
  ...
  static async htmlIncludeWithSearchParametersHttpsServerRead() {
    ...
    try {
      ...
      let renderer, rslt, cases, pi, dom, els, label;
      for (let test of tests) {
        renderer = await engine.compile(test.template);
        rslt = await renderer(files.htmlContext, opts.render);console.log(rslt,'')
        dom = new JSDOM(rslt);
        ...
        for (let cased of cases) {
          for (let name in cased.params) {
            if (cased.usesSearchParams) {
              ...
              label = `include parameter DOM element with ID: "${idPrefix}${name}"`;
            ...
            if (!cased.hasOwnProperty('usesIncludeParams') || cased.usesIncludeParams) {
              label = `include parameter DOM element with ID: "${name}"`;
